...
Top-level structure of the AUDIOLIB repository:
...
.. raw:: html
   ...
   <div class="repo-tree">
   <div class="rt-root">audiolib/</div>
   ...
   <details class="rt-folder rt-l1" open>
     ...
     <div class="rt-children">
       ...
       <div class="rt-file rt-l2"><span class="rt-icon">&#128193;</span><span class="rt-name">AUDIOLIB_&lt;kernel&gt;/</span><span class="rt-desc">Per-kernel sources + public header</span></div>
     </div>
   </details>
   ...
   <details class="rt-folder rt-l1" open>
     ...
     <div class="rt-children">
       <div class="rt-file rt-l2"><span class="rt-icon">&#128193;</span><span class="rt-name">AUDIOLIB_idat_gen/</span><span class="rt-desc">Python test-vector generators</span></div>
       <div class="rt-file rt-l2"><span class="rt-icon">&#128193;</span><span class="rt-name">AUDIOLIB_&lt;kernel&gt;/</span><span class="rt-desc">Per-kernel test driver + data</span></div>
     </div>
   </details>

   <!-- examples/ (leaf folder, no children) -->
...
Kernel sources — ``src/AUDIOLIB_<kernel>/``
-------------------------------------------
...
.. list-table::
   ...
   * - ``AUDIOLIB_<kernel>_cn.cpp``
     - C-native reference implementation (``_cn`` / ``NATC``)
   * - ``AUDIOLIB_<kernel>_ci.cpp``
     - C7™ intrinsic-optimized implementation (``_ci`` / ``OPT``)
   * - ``AUDIOLIB_<kernel>_priv.h``, ``*_common.*``
     - Internal helpers and shared code — not part of the public API
   * - ``CMakeLists.txt``
     - Build rules for the kernel

``src/common/`` holds cross-kernel definitions: ``AUDIOLIB_types.h`` (status
codes, kernel handle, function-style enums), ``AUDIOLIB_bufParams.h``,
``AUDIOLIB_inlines.h``, ``AUDIOLIB_utility.h``, and ``TI_platforms.h``.

Tests — ``test/AUDIOLIB_<kernel>/``
------------------------------------
...
.. list-table::
   ...
   * - File
     - Contents
   * - ``AUDIOLIB_<kernel>_d.c``
     - Test driver (entry point) for the kernel
   * - ``AUDIOLIB_<kernel>_check.{cpp,h}``
     - Result verification against the reference
   * - ``AUDIOLIB_<kernel>_idat.{c,h}``
     - Generated test vectors (see :doc:`getting_started`)

Test-vector generators — ``test/AUDIOLIB_idat_gen/<kernel>/``
--------------------------------------------------------------
...
- ``examples/AUDIOLIB_<kernel>/`` — a minimal, self-contained usage example per kernel.
