...
#
# This file is part of the AUDIOLIB documentation build.
#
...
# been omitted — AUDIOLIB is a single-target C library and does not need it.
#
# Doxygen XML is produced by the CMake build (not by Exhale).  Pass the
# output directory via the AUDIOLIB_DOXYGEN_XML environment variable before
# invoking sphinx-build, e.g.:
#   export AUDIOLIB_DOXYGEN_XML=/path/to/build/docs/xml
...
# (AUDIOLIB_DOC_REF, set by CI) -> current git branch -> "main".
import subprocess


def _doc_ref():
    ref = os.environ.get("AUDIOLIB_DOC_REF")
    if ref:
        return ref.strip()
    try:
        out = subprocess.run(
...
xml_dir = os.environ.get("AUDIOLIB_DOXYGEN_XML", _fallback_xml_dir)

if not os.path.isdir(xml_dir):
    import warnings
    warnings.warn(
        f"[conf.py] Doxygen XML directory not found: {xml_dir!r}\n"
        "  Set AUDIOLIB_DOXYGEN_XML to the path produced by the CMake 'doxygen' target, "
        "or run CMake first to generate the XML before invoking sphinx-build.",
        stacklevel=2,
    )


def _strip_latex_images(xdir):
    """Remove ``@image latex`` nodes from the generated Doxygen XML.

    Breathe renders latex image nodes in HTML output too, producing broken
    <img> links to non-existent .pdf files (e.g. AUDIOLIB_nlms_block_diagram.pdf).
    HTML docs only need the ``@image html`` (svg) variant. The XML is generated
    and ephemeral, so rewriting it here (before Breathe parses it) is safe.
    """
    import re
    import glob
    if not os.path.isdir(xdir):
        return
    pat_pair = re.compile(r'<image\s+type="latex".*?</image>', re.S)
    pat_self = re.compile(r'<image\s+type="latex"[^>]*/>')
    for path in glob.glob(os.path.join(xdir, "*.xml")):
        try:
            text = open(path, encoding="utf-8").read()
        except OSError:
            continue
        new = pat_self.sub("", pat_pair.sub("", text))
        if new != text:
            open(path, "w", encoding="utf-8").write(new)


_strip_latex_images(xml_dir)
...
# Use the system Graphviz explicitly (renders the AUDIOLIB_router \dot diagram).
...
breathe_projects        = {"audiolib": xml_dir}
breathe_default_project = "audiolib"
# AUDIOLIB is C, not C++. Parse .h/.c as the C domain so typedef'd handle
# signatures (e.g. AUDIOLIB_kernelHandle) resolve instead of tripping the
...
# ---------------------------------------------------------------------------
# AUDIOLIB organizes each kernel as a Doxygen group (\defgroup / \addtogroup).
...
_API_GROUP_ORDER = [
    ...
    ("Gain & Level", [
        "AUDIOLIB_gain",
        "AUDIOLIB_gainNCh",
        "AUDIOLIB_gainNChTrim",
        "AUDIOLIB_balance",
        "AUDIOLIB_balanceFader",
        "AUDIOLIB_mute",
        "AUDIOLIB_muteNCh",
    ]),
    ("Conversion & Measurement", [
        ...
        "AUDIOLIB_undB10",
        "AUDIOLIB_undB20",
        "AUDIOLIB_typeConversion",
        "AUDIOLIB_blockStatistics",
        "AUDIOLIB_subBlockStatistics",
    ]),
    ("Effects & Processing", [
        "AUDIOLIB_delay",
        "AUDIOLIB_delayNChannel",
        "AUDIOLIB_crossfade",
        "AUDIOLIB_softClip",
        "AUDIOLIB_nlms",
    ]),
    ...
    ("Sample Rate & Synthesis", [
        "AUDIOLIB_asrc",
        "AUDIOLIB_ssrc",
        "AUDIOLIB_sinusoidGenerator",
        "AUDIOLIB_whiteNoiseGenerator",
    ]),
    ("Lookup & Interpolation", [
        "AUDIOLIB_tableInterpolate",
        "AUDIOLIB_tableLookup",
    ]),
]


def _caption_to_slug(caption):
    """Convert a caption like 'Gain & Level' to a slug like 'gain_level'."""
    import re
...
def _generate_api_pages(xml_directory, api_directory):
    ...
    Layout (mirroring TISP immaterial approach):
    - One page per kernel group: ``api/<group>.rst`` (doxygengroup + :members:)
    - Landing pages per caption: ``api/<slug>.rst`` (toctree of that caption's groups)
    - ``api/index.rst``: single toctree listing all landing pages in order
      (no :caption: — sphinx-immaterial renders each landing page as a nav section)
    """
    ...
    for name, title in groups:
        ...
        with open(os.path.join(api_directory, f"{name}.rst"), "w",
            ...
            fh.write(f"{title}\n{underline}\n\n")
            if name in _NO_MEMBERS_GROUPS:
                # Unbalanced Doxygen group: wrap in a temporary C namespace
                # so leaked symbols get unique qualified names (avoids
                # duplicate-declaration errors in the Sphinx C domain).
                fh.write(f".. c:namespace:: {name}\n\n")
                fh.write(f".. doxygengroup:: {name}\n\n")
                fh.write(".. c:namespace:: 0\n")
            else:
                fh.write(f".. doxygengroup:: {name}\n   :members:\n")
    ...
    # -----------------------------------------------------------------------
    # Build landing pages per caption (one .rst per category).
    # Each landing page has a toctree of its kernel group pages.
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Stale .rst cleanup: remove files from a previous generation that are
    # no longer emitted (e.g. old landing slugs after a rename).
    # -----------------------------------------------------------------------
    all_emitted = set(discovered)  # per-kernel pages
    for slug_entry in landing_index_entries:
        all_emitted.add(slug_entry)
    ...
    # -----------------------------------------------------------------------
    # api/index.rst: single toctree (:maxdepth: 2, no :caption:) listing all
    # landing pages — sphinx-immaterial uses the landing page titles for nav.
    # -----------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# No custom macros needed for AUDIOLIB — rely on MathJax 3 defaults.

pygments_style = "friendly"
# No maximum_signature_line_length: let signatures render single-line in the
# gray sig block (natural wrap) — clean look matching TISP.

# ---------------------------------------------------------------------------
# Warning suppression
# ---------------------------------------------------------------------------
# Suppress known non-critical warnings that would otherwise fail -W builds:
#
# - duplicate.c_declaration: multiple kernel groups each embed a ``funcStyle``
#   member (``int8_t funcStyle``) from the per-kernel InitArgs struct. Breathe
#   emits identical C declarations across groups; Sphinx flags them as
#   duplicates. These are a Breathe/Sphinx artefact — the rendered content is
#   correct. Follow-up: upstream fix to Breathe or qualify the declaration.
#
# - image.not_readable: AUDIOLIB_nlms embeds SVG block diagrams whose path
#   Doxygen stores relative to the XML output dir; Breathe cannot resolve
#   them from there. The images render fine in the legacy HTML Doxygen output.
#   Follow-up: copy referenced images into the XML dir during the build.
#
# - misc.highlighting_failure: occasional lexer failures on C snippets inside
#   Doxygen verbatim blocks.
#
# - ref.ref_type_mismatch: some \ref targets resolve to a different domain
#   than the calling context; harmless for navigation.

suppress_warnings = [
    "image.not_readable",
    "misc.highlighting_failure",
    "ref.ref_type_mismatch",
    "app.add_node",
]
...
html_title        = "AUDIOLIB Documentation"
...
html_theme_options = {
    # Palette: TI brand — red primary, teal accent, single light scheme.
    "palette": [
        {
            "scheme": "default",
            "primary": "red",
    ...
    "repo_url":  "https://github.com/TexasInstruments/c7x_audiolib",
    "repo_name": "c7x_audiolib",
...
#   - C API signature token colouring
# ---------------------------------------------------------------------------
