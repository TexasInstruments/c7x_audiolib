...
AUDIOLIB uses **CMake presets** to manage build configurations, keeping the
...
``test/AUDIOLIB_idat_gen/<kernel>/gen_data.py`` for every kernel, emitting the
...
.. code-block:: bash
   ...
   bin/release/test_AUDIOLIB_<kernel>_<device>_x86_64
