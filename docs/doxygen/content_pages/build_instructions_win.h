...
/**
 ...
 \page build_instructions_win Windows Build Instructions for AUDIOLIB
 ...
 \par Build
  ...
  - CMake commands can be run within the Developer PowerShell for VS 2022 at the top level "audiolib" directory
  - Build options:
...
delete build folder before rebuilding
 ...
 \par Run
  ...
  - To run AUDIOLIB kernels on a PC:
    - \code bin/Release/test_AUDIOLIB_<kernel_name>_<device>_x86_64.exe \endcode OR \code
bin/Debug/test_AUDIOLIB_<kernel_name>_<device>_x86_64.exe \endcode
      - Ex: \code bin/Release/test_AUDIOLIB_sqr_C7100_x86_64.exe \endcode
...
-DDEVICE="C7120" -DAUDIOLIB_DEBUGPRINT="1" -DTEST_CASE="2" -DCMAKE_EXPORT_COMPILE_COMMANDS="TRUE" `
-DCMAKE_BUILD_TYPE="Release" \endcode
...
delete build folder before rebuilding
 \par Run
  ...
  - To run AUDIOLIB kernels on a PC:
    - \code bin/Release/test_AUDIOLIB_<kernel_name>_<device>_x86_64.exe \endcode OR \code
bin/Debug/test_AUDIOLIB_<kernel_name>_<device>_x86_64.exe \endcode
      - Ex: \code bin/Release/test_AUDIOLIB_sqr_C7120_x86_64.exe \endcode
  - To run AUDIOLIB kernels for target
    - To reproduce numbers given in the datasheet, run the .out program on the EVM in No Boot Mode
...
/**
 ...
 \par Build
  ...
  - Build options:
    ...
    - AUTO_TEST=1/0 (builds all test cases and libraries)
  - Example CMake commands
    - To build AUDIOLIB_add for target on AM62A and run performance test cases:
      - \code cmake -B build -DTARGET_PLATFORM="" -DBUILD_TEST="1" -DKERNEL_NAME="AUDIOLIB_add" -DSOC="AM62A" `
-DDEVICE="C7504" -DAUDIOLIB_DEBUGPRINT="0" -DALL_TEST_CASES="1" -DCMAKE_EXPORT_COMPILE_COMMANDS="TRUE" `
-DCMAKE_BUILD_TYPE="Release" \endcode
    - To build AUDIOLIB_dotprod for host emulation on AM62A and run example code
      - \code cmake -B build -DTARGET_PLATFORM="PC" -DBUILD_EXAMPLE="1" -DKERNEL_NAME="AUDIOLIB_dotprod" `
-DSOC="AM62A" -DDEVICE="C7504" -DAUDIOLIB_DEBUGPRINT="0" -DCMAKE_EXPORT_COMPILE_COMMANDS="TRUE" `
-DCMAKE_BUILD_TYPE="Release" \endcode
    - To build AUDIOLIB_max for host emulation on AM62A and run test case 2 with debug print statements
        - \code cmake -B build -DTARGET_PLATFORM="PC" -DBUILD_TEST="1" -DKERNEL_NAME="AUDIOLIB_max" -DSOC="AM62A" `
-DDEVICE="C7504" -DAUDIOLIB_DEBUGPRINT="1" -DTEST_CASE="2" -DCMAKE_EXPORT_COMPILE_COMMANDS="TRUE" `
-DCMAKE_BUILD_TYPE="Release" \endcode
...
delete build folder before rebuilding
 ...
 \par Run
  ...
  - To run AUDIOLIB kernels on a PC:
    - \code bin/Release/test_AUDIOLIB_<kernel_name>_<device>_x86_64.exe \endcode OR \code
bin/Debug/test_AUDIOLIB_<kernel_name>_<device>_x86_64.exe \endcode
      - Ex: \code bin/Release/test_AUDIOLIB_sqr_C7504_x86_64.exe \endcode
  - To run AUDIOLIB kernels for target
    - To reproduce numbers given in the datasheet, run the .out program on the EVM in No Boot Mode
