...
int AUDIOLIB_router_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_router_testParams_t *prm;
   AUDIOLIB_router_testParams_t  currPrm;
   AUDIOLIB_router_getTestParams(&prm, &test_cases);

   AUDIOLIB_router_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 0\n");

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 1\n");

   TI_profile_init("AUDIOLIB_router");
   // file IO for EVM benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_router.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      kerInitArgs.numOutputs   = currPrm.outChannels;
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (isInterleave == 1) {
            ...
            bufParamsOut.dim_x     = currPrm.outChannels;
            ...
            bufParamsOut.dim_y     = currPrm.outChannels;
            bufParamsOut.stride_y  = currPrm.strideOut;
         }

         bufParamsIn1.data_type = currPrm.dataType;
         bufParamsIn1.dim_x     = currPrm.outChannels;
         ...
         int32_t dim_y_padded  = AUDIOLIB_ROW_STRIDE(bufParamsIn0.dim_y, eleCount);
         int32_t dim_y_padded1 = AUDIOLIB_ROW_STRIDE(bufParamsOut.dim_x, eleCount);

         uint32_t inp0SizeBytes, inp1SizeBytes, outSizeBytes, outSizeBytes1;

         if (isInterleave) {

            inp0SizeBytes = dim_y_padded * bufParamsIn0.stride_y;
            inp1SizeBytes = bufParamsIn1.dim_x * dataWidth;
            outSizeBytes  = bufParamsOut.dim_y * bufParamsOut.stride_y;
            outSizeBytes1 = dim_y_padded1 * currPrm.inSamples * dataWidth;
         }
         else {
         ...
         if (pIn0 && pIn1 && pOut && pOutCn && pOutScratch) {
            ...
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
               TI_fillBuffer_float(prm[tpi].testPattern, 0, pIn0, prm[tpi].staticIn0, bufParamsIn0.dim_x,
                                   ...
                                   testPatternString);
               TI_fillBuffer_float(prm[tpi].testPattern, 0, pIn1, prm[tpi].staticIn1, currPrm.outChannels, 1, 0,
            ...
            status_init =
                AUDIOLIB_router_init_checkParams(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 1 status_init %d\n", status_init);

            if (status_init == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               AUDIOLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
               status_init = AUDIOLIB_router_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_router_set(handle, pIn1);

               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 2 status_init %d\n", status_init);

            status_opt = AUDIOLIB_router_exec_checkParams(handle, pIn0, pIn1, pOut, pOutScratch);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 3 status_opt %d\n", status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_router_exec(handle, pIn0, pOut, pOutScratch);
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 4 status_opt %d\n", status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_router_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);
            AUDIOLIB_router_set(handle, pIn1);

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_router_exec(handle, pIn0, pOutCn, pOutScratch);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0,
                                  "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 8 status_nat_vs_opt %d status_ref_vs_opt "
                                  "%d currentTestFail "
                                  "%d\n",
                                  status_nat_vs_opt, status_ref_vs_opt, currentTestFail);

            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_router_d CP 8 fail %d\n", fail);
            ...
            AUDIOLIB_router_perfEst(handle, &archCycles, &estCycles);
            fprintf(fpOutputCSV, "%d, %d, %d,%d, %d, %d ,%ld, %d\n", currPrm.testID,
                    AUDIOLIB_sizeof(currPrm.dataType) * 8, currPrm.isInterleave, currPrm.inSamples, currPrm.inChannels,
            ...
            // clear the counters between runs; normally handled by TI_profile_router_test
...
#endif
   {
      return AUDIOLIB_router_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1005) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_router_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp,
                                                       &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt = AUDIOLIB_router_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp,
                                                       &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_router_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp,
                                                       &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt = AUDIOLIB_router_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp,
                                                       &kerInitArgs);
      ...
      case 1004:
         /* Valid handle and buffers but NULL InitArgs -> NULL_POINTER
          * (covers the pKerInitArgs operand of the init_checkParams null-check). */
         bufParamsIn0Temp.data_type = bufParamsIn0.data_type;
         bufParamsIn1Temp.data_type = bufParamsIn1.data_type;
         bufParamsOutTemp.data_type = bufParamsOut.data_type;
         ...
         status_opt =
             AUDIOLIB_router_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, NULL);
         currentTestFail = (status_opt != AUDIOLIB_ERR_NULL_POINTER);
         break;

      case 1005:
         /* exec_checkParams: NULL handle -> NULL_POINTER (TRUE branch);
          * all-valid pointers -> SUCCESS (FALSE branch). */
         status_nat      = AUDIOLIB_router_exec_checkParams(NULL, pIn0, pIn1, pOut, pOut);
         status_opt      = AUDIOLIB_router_exec_checkParams(handle, pIn0, pIn1, pOut, pOut);
   ...
   /* For every call of AUDIOLIB_router_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_router_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_router_getHandleSize(&kerInitArgs);
...
/* Main call for inrouteridual test projects */
