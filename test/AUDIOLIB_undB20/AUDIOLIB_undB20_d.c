...
#include "AUDIOLIB_undB20_idat.h"
...
int AUDIOLIB_undB20_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t                tpi;
   ...
   AUDIOLIB_undB20_testParams_t *prm;
   AUDIOLIB_undB20_testParams_t  currPrm;
   AUDIOLIB_undB20_getTestParams(&prm, &test_cases);

   AUDIOLIB_undB20_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 0\n");

   int32_t               handleSize = AUDIOLIB_undB20_getHandleSize(&kerInitArgs);
   AUDIOLIB_kernelHandle handle     = malloc(handleSize);

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 1\n");

   TI_profile_init("AUDIOLIB_undB20");

   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_undB20.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            status_init = AUDIOLIB_undB20_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 1 status_init %d\n", status_init);

            if (status_init == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               AUDIOLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
               status_init           = AUDIOLIB_undB20_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }
            ...
            status_opt = AUDIOLIB_undB20_exec_checkParams(handle, pIn, pOut);
#if AUDIOLIB_DEBUGPRINT
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 3 status_opt %d\n", status_opt);
#endif
            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_undB20_exec(handle, pIn, pOut);
               AUDIOLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 4 status_opt %d\n", status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_undB20_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_undB20_exec(handle, pIn, pOutCn);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == AUDIOLIB_FLOAT32) {
            ...
            comparisonDone = 1;
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
...
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d "
                   "currentTestFail %d\n",
...
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_undB20_d CP 8 fail %d\n", fail);
#endif
            pProfile[3 * tpi]     = (int32_t) TI_profile_get_cycles(TI_PROFILE_KERNEL_OPT);
            pProfile[3 * tpi + 1] = (int32_t) TI_profile_get_cycles(TI_PROFILE_KERNEL_OPT_WARM);
            pProfile[3 * tpi + 2] = (int32_t) TI_profile_get_cycles(TI_PROFILE_KERNEL_OPT_WARMWRB);

            AUDIOLIB_undB20_perfEst(handle, &archCycles, &estCycles);
...
#endif
   {
      return AUDIOLIB_undB20_d(&pProfile[0], 0);
   }
}
int coverage_test_main()
{
   ...
   while (testNum <= 1005) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_undB20_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt      = AUDIOLIB_undB20_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_undB20_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt      = AUDIOLIB_undB20_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1003:
         ...
         /* Sample Values */
         ((float *) pIn0)[0] = -759; // underflow

         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_undB20_init(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);

         status_exec_nat = AUDIOLIB_undB20_exec(handle, pIn0, pOutCn);

         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_undB20_init(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);

         status_exec_opt = AUDIOLIB_undB20_exec(handle, pIn0, pOut);
   ...
   /* For every call of AUDIOLIB_undB20_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_undB20_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_undB20_getHandleSize(&kerInitArgs);
