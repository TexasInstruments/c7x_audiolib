...
#else
static uint8_t sram_start_placeholder __attribute__((section(".sram_start")));
#endif
                                       ...
                                       uint8_t  elementSize)
{
   ...
   for (row = 0; row < height; row++) {
      for (col = 0; col < width; col++) {
         ...
         int32_t valA, valB;
#else
         int32_t valA = 0, valB = 0; // Initialize valA and valB
#endif
         int32_t offsetA, offsetB;
                                ...
                                uint8_t  elementSize)
{
   ...
   for (row = 0; row < height; row++) {
      for (col = 0; col < width; col++) {
         ...
         // NaN check for floating ponits
         if ((valA != valA) || (valB != valB)) {
#ifdef C7X
            sprintf(TI_test_print_buffer, "\nFailure at: row=%d, col=%d, val1=%e, val2=%e (NaN ERROR)\n", row, col,
                    valA, valB);
#else
            DebugP_log("Failure at: row=%d, col=%d, val1=%e, val2=%e (NaN ERROR)\r\n", row, col, valA, valB);
#endif
            TI_test_print_string(TI_test_print_buffer);
            return (TI_TEST_KERNEL_FAIL);
         }
         ...
         if (valA != valB) {
            if (log10tolerance) {
               bool log10diffCheck = true;
               bool absdiffCheck   = true;
               // Do not perform log10 check if valA is 0
               if (valA != 0) {
                  double log10diff = fabs(log10(fabs(valB / valA)));
                  log10diffCheck   = log10diff > log10tolerance;
               }
               double absdiff = fabs(valB - valA);
               absdiffCheck   = absdiff > abstolerance;

               if (log10diffCheck && absdiffCheck) {
#ifdef C7X
                  sprintf(TI_test_print_buffer, "\nFailure at: row=%d, col=%d, val1=%e, val2=%e\n", row, col, valA,
                          valB);
#else
                  DebugP_log("Failure at: row=%d, col=%d, val1=%e, val2=%e\r\n", row, col, valA, valB);
#endif
                  TI_test_print_string(TI_test_print_buffer);
                  return (TI_TEST_KERNEL_FAIL);
               }
               else if (warn == 0) {
#if (!AUDIOLIB_COMPARE_SUPPRESS_WARNINGS)
                  warn = 1;
#ifdef C7X
                  sprintf(TI_test_print_buffer, "\nWarning at: row=%d, col=%d, val1=%e, val2=%e\n", row, col, valA,
                          valB);
#else
                  DebugP_log("Warning at: row=%d, col=%d, val1=%e, val2=%e \r\n", row, col, valA, valB);
#endif
                  TI_test_print_string(TI_test_print_buffer);
#endif
               }
...
/* Common Memory Utility:
 *
 *    TI_compare_mem_2DDualStride_float
 *
 */
   ...
   for (row = 0; row < height; row++) {
      for (col = 0; col < width; col++) {
         ...
         if (elementSize == 4) {
            ...
            sprintf(TI_test_print_buffer, "\nUnsupported data type in TI_compare_mem_2D_float.\n");
#else
            DebugP_log("Unsupported data type in TI_compare_mem_2D_float.\r\n");
#endif
            TI_test_print_string(TI_test_print_buffer);
         }
...
double TI_calculate_snr_2DDualStride_float(void    *a,
                                           void    *b,
                                           uint32_t width,
                                           uint32_t height,
                                           uint32_t strideA,
                                           ...
                                           uint8_t  elementSize)
{
   ...
   for (row = 0; row < height; row++) {
      for (col = 0; col < width; col++) {
         ...
         int32_t offsetA = (row * (strideA / elementSize)) + col;
         int32_t offsetB = (row * (strideB / elementSize)) + col;
         ...
         if (elementSize == 4) {
            ...
            printf("\nUnsupported data type in calculate_snr.\n");
            return -1.0; // Return error for unsupported data type
         }

         // Accumulate signal power and noise power
         signal_power += valA * valA;
         double noise = valA - valB;
         noise_power += noise * noise;
      }
   }

   // Calculate mean squared values
   signal_power /= (width * height);
   noise_power /= (width * height);

   // Avoid division by zero
   if (noise_power == 0.0) {
      snr = 1.0E30; // Infinite SNR if there's no noise
   }
   else {
      // Calculate SNR in dB
...
// assume the stride is the same as the number of cols, as when data is loaded
// from a static array
int32_t TI_compare_mem_roi2DStatic_float(void    *a,
                                         void    *b,
                                         double   log10tolerance,
                                         double   abstolerance,
                                         uint32_t width,
                                         ...
                                         uint8_t  elementSize)
{
   return TI_compare_mem_2DDualStride_float(a, b, log10tolerance, abstolerance, width, height, stride,
   ...
   for (ctrZ = 0; ctrZ < dim_z; ctrZ++) {
      for (ctrY = 0; ctrY < dim_y; ctrY++) {
         for (ctrX = 0; ctrX < dim_x; ctrX++) {
            ...
            // NaN check for floating ponits
            if ((valA != valA) || (valB != valB)) {
               sprintf(TI_test_print_buffer, "\nFailure at: z=%d, y=%d, x=%d, val1=%e, val2=%e (NaN ERROR)\n", ctrZ,
                       ctrY, ctrX, valA, valB);
               TI_test_print_string(TI_test_print_buffer);
               return (TI_TEST_KERNEL_FAIL);
            }
            ...
            if (valA != valB) {
               if (log10tolerance) {
                  ...
                  if (log10diffCheck && absdiffCheck) {
                     sprintf(TI_test_print_buffer, "\nFailure at: z=%d, y=%d, x=%d, val1=%e, val2=%e\n", ctrZ, ctrY,
                             ctrX, valA, valB);
                     TI_test_print_string(TI_test_print_buffer);
                     return (TI_TEST_KERNEL_FAIL);
                  }
                     ...
                     warn = 1;
                     sprintf(TI_test_print_buffer, "\nWarning at: z=%d, y=%d, x=%d, val1=%e, val2=%e\n", ctrZ, ctrY,
                             ctrX, valA, valB);
                     TI_test_print_string(TI_test_print_buffer);
                  }
               }
...
int32_t TI_compare_mem_3D_float_dualStride(void    *a,
                                           void    *b,
                                           double   log10tolerance,
                                           double   abstolerance,
                                           uint32_t dim_x,
   ...
   for (ch = 0; ch < dim_z; ch++) {
      for (row = 0; row < dim_y; row++) {
         for (col = 0; col < dim_x; col++) {
            ...
            offsetB = ch * (stride_zB / elementSize) + (row * (stride_yB / elementSize)) + col;
            ...
            // NaN check for floating ponits
            if ((valA != valA) || (valB != valB)) {
               sprintf(TI_test_print_buffer, "\nFailure at: ch=%d, row=%d, col=%d, val1=%e, val2=%e (NaN ERROR)\n", ch,
                       row, col, valA, valB);
               TI_test_print_string(TI_test_print_buffer);
               return (TI_TEST_KERNEL_FAIL);
            }
            ...
            if (valA != valB) {
               if (log10tolerance) {
                  ...
                  if (log10diffCheck && absdiffCheck) {
                     sprintf(TI_test_print_buffer, "\nFailure at: ch=%d, row=%d, col=%d, val1=%e, val2=%e\n", ch, row,
                             col, valA, valB);
                     TI_test_print_string(TI_test_print_buffer);
                     return (TI_TEST_KERNEL_FAIL);
                  }
                     ...
                     warn = 1;
                     sprintf(TI_test_print_buffer, "\nWarning at: ch=%d, row=%d, col=%d, val1=%e, val2=%e\n", ch, row,
                             col, valA, valB);
                     TI_test_print_string(TI_test_print_buffer);
                  }
               }
                              ...
                              uint8_t  elementSize)
{
   if (height == width) {
      ...
      if (elementSize == 4) {
         float *temp_ptr =
             (float *) temp; //(float *)TI_memalign(AUDIOLIB_L2DATA_ALIGNMENT, height * width * elementSize);
         ...
         double *temp_ptr =
             (double *) temp; //(double *)TI_memalign(AUDIOLIB_L2DATA_ALIGNMENT, height * width * elementSize);
...
/* 2D Memory Utility:
 *
 *    copyRoi_float_circular
 *    Handles copying to a circular buffer where data might wrap around the end
 *
 */
void copyRoi_float_circular(void    *arrayDst,
                            void    *arraySrc,
                            uint32_t width,
                            uint32_t height,
                            uint32_t stride,
                            ...
                            uint8_t  elementSize)
{
   ...
   if (elementSize == 4) {
      float *restrict dst     = (float *) arrayDst;
      float *restrict src     = (float *) arraySrc;
      uint32_t bufferElements = bufferSize / elementSize;

      for (row = 0; row < height; row++) {
         for (col = 0; col < width; col++) {
            // Calculate destination index with circular wrapping
            dstIdx      = (startOffset + row * stride / 4 + col) % bufferElements;
            dst[dstIdx] = src[row * width + col];
         }
      }
   }
   else if (elementSize == 8) {
      ...
      for (row = 0; row < height; row++) {
         for (col = 0; col < width; col++) {
            // Calculate destination index with circular wrapping
            dstIdx      = (startOffset + row * stride / 8 + col) % bufferElements;
            dst[dstIdx] = src[row * width + col];
         }
      }
   }
   else {
      sprintf(TI_test_print_buffer, "\nUnsupported data type in copyRoi_float_circular.\n");
      TI_test_print_string(TI_test_print_buffer);
   }
}
...
/* 2D Memory Utility:
 *
 *    copyRoiDualStride_float_circular
 *    Handles copying to a circular buffer with different strides where data might wrap around the end
 *    Used for non-interleaved data format
 *
 */
void copyRoiDualStride_float_circular(void    *arrayDst,
                                      void    *arraySrc,
                                      uint32_t width,
                                      uint32_t height,
                                      uint32_t strideDst,
                                      ...
                                      uint8_t  elementSize)
{
   ...
   if (elementSize == 4) {
      ...
      for (row = 0; row < height; row++) {
         for (col = 0; col < width; col++) {
            // Calculate destination index with circular wrapping
            dstIdx      = (startOffset + row * strideDst / 4 + col) % bufferElements;
            dst[dstIdx] = src[row * strideSrc / 4 + col];
         }
      }
   }
   else if (elementSize == 8) {
      ...
      for (row = 0; row < height; row++) {
         for (col = 0; col < width; col++) {
            // Calculate destination index with circular wrapping
            dstIdx      = (startOffset + row * strideDst / 8 + col) % bufferElements;
            dst[dstIdx] = src[row * strideSrc / 8 + col];
         }
      }
   }
   else {
      sprintf(TI_test_print_buffer, "\nUnsupported data type in copyRoiDualStride_float_circular.\n");
      TI_test_print_string(TI_test_print_buffer);
   }
}
...
/* 2D Memory Utility:
 *
 *    copyRoiDualStride_float
 *
 */
void copyRoiDualStride_float(void    *arrayDst,
                             void    *arraySrc,
                             uint32_t width,
                             uint32_t height,
                             uint32_t strideDst,
                             ...
                             uint8_t  elementSize)
{
   ...
   if (elementSize == 4) {
      ...
      sprintf(TI_test_print_buffer, "\nUnsupported data type in copyRoiDualStride_float.\n");
      TI_test_print_string(TI_test_print_buffer);
   }
}
 ...
 *
 *    calculateSNR
 *    - Calculate total SNR across all output channels
 *
 */

double calculateSNR(float *dstRef, float *dstOpt, uint32_t blockSize, uint32_t numChannels, uint32_t strideElements)
{
   ...
   for (c = 0; c < numChannels; c++) {
      ...
      uint32_t idx = c * strideElements;

      for (i = 0; i < blockSize; i++) {

         /* calculate the power of the signal (reference) */
         pSignal += (double) (dstRef[idx + i]) * (double) (dstRef[idx + i]);
         /* calculate the power of the noise (difference of reference and optimized) */
         pNoise += ((double) (dstRef[idx + i] - dstOpt[idx + i])) * ((double) (dstRef[idx + i] - dstOpt[idx + i]));
      }
   }
