...
#else
#define PROFILE_INIT
#define TSCH 0 // Replace with the actual high part of the timestamp counter
#define TSCL 0 // Replace with the actual low part of the timestamp counter
#define PROFILE_READ (read_cycle_counter())
static inline uint64_t read_cycle_counter(void)
{
   uint64_t t;
   __asm__ __volatile__("dsb sy" ::: "memory");
   __asm__ __volatile__("isb");
   __asm__ __volatile__("mrs %0, pmccntr_el0" : "=r"(t));
   return t;
}
#endif
#endif
...
#else
   cycles[act_kernel] += end_count - beg_count;
#endif
