...
#include "AUDIOLIB_blockStatistics_idat.h"
...
int AUDIOLIB_blockStatistics_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_blockStatistics_testParams_t *prm;
   AUDIOLIB_blockStatistics_testParams_t  currPrm;
   AUDIOLIB_blockStatistics_getTestParams(&prm, &test_cases);

   AUDIOLIB_blockStatistics_InitArgs kerInitArgs;
   AUDIOLIB_blockStatistics_SetArgs  kerSetArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 0\n");

   int32_t               handleSize = AUDIOLIB_blockStatistics_getHandleSize(&kerInitArgs);
   AUDIOLIB_kernelHandle handle     = malloc(handleSize);

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 1\n");

   TI_profile_init("AUDIOLIB_blockStatistics");
   // #if (!defined(EVM_TEST))
   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_blockStatistics.csv", "w+");
   fprintf(fpOutputCSV, "Test ID, Bit Width, inSamples, inChannels, statisticsType, Arch cycles, "
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            status_init = AUDIOLIB_blockStatistics_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 1 status_init %d\n",
                                  status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               status_init           = AUDIOLIB_blockStatistics_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_blockStatistics_set(handle, &kerSetArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 2 status_init %d\n",
                                  status_init);
            status_opt = AUDIOLIB_blockStatistics_exec_checkParams(handle, pIn, pOut);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 3 status_opt %d\n",
                                  status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_blockStatistics_exec(handle, pIn, pOut);
               AUDIOLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 4 status_opt %d\n",
                                  status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_blockStatistics_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
            AUDIOLIB_blockStatistics_set(handle, &kerSetArgs);
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_blockStatistics_exec(handle, pIn, pOutCn);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0,
                                  "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 8 status_nat_vs_opt %d "
                                  "status_ref_vs_opt %d currentTestFail "
                                  "%d\n",
                                  status_nat_vs_opt, status_ref_vs_opt, currentTestFail);

            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_blockStatistics_d CP 8 fail %d\n", fail);

            pProfile[3 * tpi]     = (int32_t) TI_profile_get_cycles(TI_PROFILE_KERNEL_OPT);
            pProfile[3 * tpi + 1] = (int32_t) TI_profile_get_cycles(TI_PROFILE_KERNEL_OPT_WARM);
            pProfile[3 * tpi + 2] = (int32_t) TI_profile_get_cycles(TI_PROFILE_KERNEL_OPT_WARMWRB);

            AUDIOLIB_blockStatistics_perfEst(handle, &archCycles, &estCycles);
            AUDIOLIB_blockStatistics_get(handle, &kerSetArgs);
            sprintf(desc, "%s generated input | inSamples = %d, inChannels = %d, statisticsType = %d",
                    testPatternString, currPrm.inputBlockSize, currPrm.numChannels, kerSetArgs.statisticsType);
            // #if (!defined(EVM_TEST))
            // write to CSV, must happen prior to write to screen because
            // TI_profile_formula_add clears values in counters
            fprintf(fpOutputCSV, "%d, %d, %d, %d, %d, %d, %d, %f, %d\n", currPrm.testID,
                    AUDIOLIB_sizeof(currPrm.dataType) * 8, currPrm.inputBlockSize, currPrm.numChannels,
                    currPrm.statisticsType, (int32_t) estCycles, pProfile[3 * tpi + 1],
            ...
            // clear the counters between runs; normally handled by TI_profile_BlockStatistics_test
...
int test_main(uint32_t *pProfile)
{
   ...
   if (TI_cache_init()) {
      TI_memError("AUDIOLIB_blockStatistics");
      return 1;
   }
   else
#else
...
#endif
   {
      return AUDIOLIB_blockStatistics_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1002) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat =
             AUDIOLIB_blockStatistics_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt =
             AUDIOLIB_blockStatistics_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat =
             AUDIOLIB_blockStatistics_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt =
             AUDIOLIB_blockStatistics_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
   ...
   /* For every call of AUDIOLIB_blockStatistics_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_blockStatistics_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_blockStatistics_getHandleSize(&kerInitArgs);
   printf("!!! Pushing final execution history. handleSize_LDRA: %d\n", handleSize_LDRA);
#endif

   return fail;
}
/* Main call for inBlockStatisticsidual test projects */
