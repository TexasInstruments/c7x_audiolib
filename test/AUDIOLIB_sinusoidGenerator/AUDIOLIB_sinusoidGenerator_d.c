...
int AUDIOLIB_sinusoidGenerator_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_sinusoidGenerator_testParams_t *prm;
   AUDIOLIB_sinusoidGenerator_testParams_t  currPrm;
   AUDIOLIB_sinusoidGenerator_getTestParams(&prm, &test_cases);

   AUDIOLIB_sinusoidGenerator_InitArgs kerInitArgs;
   AUDIOLIB_sinusoidGenerator_SetArgs  kerSetArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 0\n");

   int32_t               handleSize = AUDIOLIB_sinusoidGenerator_getHandleSize(&kerInitArgs);
   AUDIOLIB_kernelHandle handle     = calloc(1, handleSize);

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 1\n");

   TI_profile_init("AUDIOLIB_sinusoidGenerator");
   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_sinusoidGenerator.csv", "w+");
   fprintf(fpOutputCSV, "Test ID, Bit Width, numSamples,est cycles,"
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      currPrm                    = prm[tpi];
      kerSetArgs.startPhase      = currPrm.startPhase;
      kerSetArgs.startFrequency  = currPrm.startFrequency;
      kerSetArgs.targetFrequency = currPrm.targetFrequency;
      kerSetArgs.smoothingTime   = currPrm.smoothingTime;
      kerSetArgs.samplingRate    = currPrm.samplingRate;
      kerInitArgs.executionMode  = currPrm.executionMode;
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pOut && pOutCn) {
            ...
            status_init = AUDIOLIB_sinusoidGenerator_init_checkParams(handle, &bufParamsOut, &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 1 status_init %d\n",
                                  status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               status_init           = AUDIOLIB_sinusoidGenerator_init(handle, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_sinusoidGenerator_set(handle, &kerSetArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 2 status_init %d\n",
                                  status_init);

            status_opt = AUDIOLIB_sinusoidGenerator_exec_checkParams(handle, pOut);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 3 status_opt %d\n",
                                  status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_sinusoidGenerator_exec(handle, pOut);
               AUDIOLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            for (k = 0; k < (numExecReps - 2); k++) {
               ...
               AUDIOLIB_asm(" MARK 4");
               status_opt =
                   AUDIOLIB_sinusoidGenerator_exec(handle, (void *) ((int8_t *) pOut + (k + 1) * outSizeBytes));
               AUDIOLIB_asm(" MARK 5");
               TI_profile_stop();
            }
            ...
            if (numExecReps != 1) {
               ...
               AUDIOLIB_asm(" MARK 6");
               status_opt = AUDIOLIB_sinusoidGenerator_exec(
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 4 status_opt %d\n",
                                  status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_sinusoidGenerator_init(handle, &bufParamsOut, &kerInitArgs);
            AUDIOLIB_sinusoidGenerator_set(handle, &kerSetArgs);
#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 5\n");
#endif
            for (k = 0; k < numExecReps; k++) {
               TI_profile_start(TI_PROFILE_KERNEL_CN);
               AUDIOLIB_asm(" MARK 8");
               status_nat = AUDIOLIB_sinusoidGenerator_exec(handle, (void *) ((int8_t *) pOutCn + k * outSizeBytes));
               AUDIOLIB_asm(" MARK 9");
               TI_profile_stop();
            }

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
               ...
               for (frame = 0; frame < numExecReps; frame++) {
                  int32_t frame_status_nat_vs_opt = TI_compare_mem_2D_float(
                      ...
                      bufParamsOut.dim_x, 1, 0, AUDIOLIB_sizeof(currPrm.dataType));
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
               ...
               if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
                  ...
                  for (frame = 0; frame < numExecReps; frame++) {
                     ...
                     int32_t frame_status_ref_vs_opt = TI_compare_mem_2DDualStride_float(
                         ...
                         (double) powf(2, -10), bufParamsOut.dim_x, 1, 0, 0, AUDIOLIB_sizeof(currPrm.dataType));
            ...
            AUDIOLIB_DEBUGPRINTFN(0,
                                  "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 8 status_nat_vs_opt %d "
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0,
                "AUDIOLIB_DEBUGPRINT  AUDIOLIB_sinusoidGenerator_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            sprintf(desc, "%s data does not fit in memory | numSamples = %d", testPatternString, currPrm.numSamples);
            TI_profile_skip_test(desc);

            // clear the counters between runs; normally handled by TI_profile_sinusoidGenerator_test
...
int test_main(uint32_t *pProfile)
{
   ...
   if (TI_cache_init()) {
      TI_memError("AUDIOLIB_sinusoidGenerator");
      return 1;
   }
   else
#else
...
#endif
   {
      return AUDIOLIB_sinusoidGenerator_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1001) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_sinusoidGenerator_init_checkParams(NULL, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_sinusoidGenerator_init_checkParams(NULL, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_sinusoidGenerator_init_checkParams(handle, &bufParamsOutTemp, &kerInitArgs);
         bufParamsOutTemp.data_type = AUDIOLIB_UINT32;

         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_sinusoidGenerator_init_checkParams(handle, &bufParamsOutTemp, &kerInitArgs);
   ...
   /* For every call of AUDIOLIB_sinusoidGenerator_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_sinusoidGenerator_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_sinusoidGenerator_getHandleSize(&kerInitArgs);
...
/* Main call for insinusoidGeneratoridual test projects */
