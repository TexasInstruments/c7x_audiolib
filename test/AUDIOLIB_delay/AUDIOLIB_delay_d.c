...
int AUDIOLIB_delay_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   AUDIOLIB_DEBUGPRINTFN(0, "%s\n", "Entering AUDIOLIB_delay_d function");
   ...
   int32_t                      eleCount       = 0;
   AUDIOLIB_delay_testParams_t *prm;
   AUDIOLIB_delay_testParams_t  currPrm;
   AUDIOLIB_delay_getTestParams(&prm, &test_cases);
   AUDIOLIB_delay_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 0\n");

   int32_t               handleSize = AUDIOLIB_delay_getHandleSize(&kerInitArgs);
   AUDIOLIB_kernelHandle handle     = malloc(handleSize);

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 1\n");

   TI_profile_init("AUDIOLIB_delay");

   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_delay.csv", "w+");
   fprintf(fpOutputCSV, "Kernel,Test ID,Test type,Bit Width,delaySize,numSamples,numChannels,Interleave,Mode,Est "
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (currPrm.mode == 0) {
            ...
            bufParamsDelay.dim_x     = currPrm.delaySize;
            bufParamsDelay.dim_y     = currPrm.numChannels;
            bufParamsDelay.stride_y  = (currPrm.delaySize + currPrm.numSamples) * AUDIOLIB_sizeof(currPrm.dataType);
         }
         else {
            while (CircularBufBaseMmemSize < (currPrm.delaySize + currPrm.numSamples)) {
               CircularBufBaseMmemSize *= 2;
            }
            bufParamsDelay.data_type = currPrm.dataType;
            bufParamsDelay.dim_x     = currPrm.delaySize + currPrm.numSamples;
            bufParamsDelay.dim_y     = currPrm.numChannels;
            bufParamsDelay.stride_y  = CircularBufBaseMmemSize * AUDIOLIB_sizeof(currPrm.dataType);
         }

         kerInitArgs.mode       = currPrm.mode;
         kerInitArgs.delaySize  = currPrm.delaySize;
         ...
         if (pIn && pDelay && pOut && pOutCn && pScratch && pDelayCn) {
            ...
            status_init =
                AUDIOLIB_delay_init_checkParams(handle, &bufParamsIn, &bufParamsDelay, &bufParamsOut, &kerInitArgs);
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 1 status_init %d\n", status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               status_init = AUDIOLIB_delay_init(handle, &bufParamsIn, &bufParamsDelay, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 2 status_init %d\n", status_init);

            status_opt = AUDIOLIB_delay_exec_checkParams(handle, pIn, pDelay, pOut, pScratch);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 3 status_opt %d\n", status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               // Execute first frame for cold cycle count
               status_opt = AUDIOLIB_delay_exec(handle, pIn, pDelay, pOut, pScratch);
               AUDIOLIB_asm(" MARK 3");
               TI_profile_stop();

               AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 4 status_opt %d\n", status_opt);
               ...
               for (frameCnt = 1; frameCnt < (numExecReps - 1); frameCnt++) {
                  ...
                  AUDIOLIB_asm(" MARK 4");
                  status_opt = AUDIOLIB_delay_exec(
               ...
               AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 5 status_opt %d\n", status_opt);
               ...
               AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 6 status_opt %d\n", status_opt);
            }
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 7 status_init %d\n", status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               for (frameCnt = 0; frameCnt < numExecReps; frameCnt++) {
                  ...
                  AUDIOLIB_asm(" MARK 8");
                  status_nat = AUDIOLIB_delay_exec(
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 8 status_nat %d\n", status_nat);
            ...
            if (status_nat == AUDIOLIB_SUCCESS) {
               ...
               if (currPrm.staticRefOutCase != NULL) {
                  ...
                  AUDIOLIB_DEBUGPRINTFN(0,
                                        "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 9 status_ref_vs_nat_out: %d "
            ...
            AUDIOLIB_DEBUGPRINTFN(0,
                                  "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 10 status_nat_vs_opt %d "
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 11 status_init %d status_opt %d status_nat %d\n",
                status_init, status_opt, status_nat);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_delay_d CP 12 fail %d\n", fail);
            ...
            AUDIOLIB_delay_perfEst(handle, &archCycles, &estCycles, currPrm.dataType);

            //  write to CSV, must happen prior to write to screen because
            //  TI_profile_formula_add clears values in counters
            fprintf(fpOutputCSV, "AUDIOLIB_delay, %d, %d, %d, %d, %d, %d, %d, %d, %ld, %ld, %f, %u\n", testNum,
                    currPrm.testPattern, AUDIOLIB_sizeof(currPrm.dataType) * 8, currPrm.delaySize, currPrm.numSamples,
            ...
            // clear the counters between runs; normally handled by TI_profile_delay_test
...
int test_main(uint32_t *pProfile)
{
   ...
   {
      return AUDIOLIB_delay_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1003) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_delay_init_checkParams(NULL, &bufParamsInTemp, &bufParamsDelayTemp, &bufParamsOutTemp,
                                                      &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt = AUDIOLIB_delay_init_checkParams(NULL, &bufParamsInTemp, &bufParamsDelayTemp, &bufParamsOutTemp,
                                                      &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_delay_init_checkParams(handle, &bufParamsInTemp, &bufParamsDelayTemp, &bufParamsOutTemp,
                                                      &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt = AUDIOLIB_delay_init_checkParams(handle, &bufParamsInTemp, &bufParamsDelayTemp, &bufParamsOutTemp,
                                                      &kerInitArgs);
   ...
   /* For every call of AUDIOLIB_delay_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_delay_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_delay_getHandleSize(&kerInitArgs);
