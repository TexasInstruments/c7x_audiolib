...
#include "AUDIOLIB_tableLookup_idat.h"
...
int AUDIOLIB_tableLookup_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_tableLookup_testParams_t *prm;
   AUDIOLIB_tableLookup_testParams_t  currPrm;
   AUDIOLIB_tableLookup_getTestParams(&prm, &test_cases);

   AUDIOLIB_tableLookup_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 0\n");

   TI_profile_init("AUDIOLIB_tableLookup");
   // file IO for EVM benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_tableLookup.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      kerInitArgs.tableLookupSize = currPrm.tableLookupSize;
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         inp1SizeBytes = bufParamsIn1.dim_x * AUDIOLIB_sizeof(bufParamsIn1.data_type);
         ...
         if (pIn0 && pIn1 && pOut && pOutCn) {
            ...
            status_init = AUDIOLIB_tableLookup_init_checkParams(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut,
                                                                &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 1 status_init %d\n", status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               status_init =
                   AUDIOLIB_tableLookup_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);
#if defined(__C7524__)
               if (currPrm.tableLookupSize >= currPrm.tableSamples) {
                  if (status_init == AUDIOLIB_SUCCESS) {
                     AUDIOLIB_tableLookup_set_ci(handle, pIn1);
                  }
               }
#endif
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 2 status_init %d\n", status_init);

            status_opt = AUDIOLIB_tableLookup_exec_checkParams(handle, pIn0, pIn1, pOut);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 3 status_opt %d\n", status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_tableLookup_exec(handle, pIn0, pIn1, pOut);
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 4 status_opt %d\n", status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_tableLookup_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_tableLookup_exec(handle, pIn0, pIn1, pOutCn);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 6 status_nat %d\n", status_nat);
#endif
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
            ...
            if (currPrm.staticOut != NULL) {
               ...
               if (currPrm.sampleDataType == AUDIOLIB_FLOAT32 || currPrm.sampleDataType == AUDIOLIB_FLOAT64) {
                  ...
                  status_ref_vs_opt = TI_compare_mem_2DDualStride_float(
                      (void *) pOut, (void *) currPrm.staticOut, 0, (double) powf(2, -10), bufParamsOut.dim_x, 1, 0,
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0,
                "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 8 status_nat_vs_opt %d status_ref_vs_opt "
                "%d currentTestFail "
                "%d\n",
                status_nat_vs_opt, status_ref_vs_opt, currentTestFail);

            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_tableLookup_d CP 8 fail %d\n", fail);
            ...
            AUDIOLIB_tableLookup_perfEst(handle, &archCycles, &estCycles, currPrm.sampleDataType,
                                         currPrm.tableLookupSize, &kerInitArgs);
            ...
            // clear the counters between runs; normally handled by TI_profile_tableLookup_test
...
int test_main(uint32_t *pProfile)
{
   ...
   if (TI_cache_init()) {
      TI_memError("AUDIOLIB_tableLookup");
      return 1;
   }
   else
#else
...
#endif
   {
      return AUDIOLIB_tableLookup_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1003) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_tableLookup_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_tableLookup_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_tableLookup_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_tableLookup_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1003:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_tableLookup_exec_checkParams(handle, NULL, pIn1, pOut);
   ...
   /* For every call of AUDIOLIB_tableLookup_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_tableLookup_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_tableLookup_getHandleSize(&kerInitArgs);
...
/* Main call for intableLookupidual test projects */
