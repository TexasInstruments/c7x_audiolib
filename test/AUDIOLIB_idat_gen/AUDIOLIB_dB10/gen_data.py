...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "AUDIOLIB_dB10_idat.h"'
    ...
    idatFile.write("static AUDIOLIB_dB10_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    # Create an instance of AUDIOLIB_dB10
    db10 = AUDIOLIB_dB10(
    ...
    if dType == "float":
        in0 = (
            np.random.uniform(minX, maxX, length)
            .astype(np.float32)
            .reshape(inChannels, inSamples)
        )
    else:
        in0 = (
            np.random.uniform(minX, maxX, length)
    ...
    # Reshape output to match input shape for header file
    out = out.reshape(inChannels, inSamples)
...
idatFileName = "../../AUDIOLIB_dB10/AUDIOLIB_dB10_idat.c"
