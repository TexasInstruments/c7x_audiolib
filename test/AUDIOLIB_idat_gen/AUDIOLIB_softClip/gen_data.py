...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    threshold = currPrm["threshold"]
    endKnee = currPrm["endKnee"]
    ...
    idatFile.write("%s, // threshold \n" % ((threshold)))
    idatFile.write("%s, // endKnee \n" % ((endKnee)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "AUDIOLIB_softClip_idat.h"'
    ...
    idatFile.write("static AUDIOLIB_softClip_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["threshold"] = float(testCase["threshold"])
    testCase["endKnee"] = float(testCase["endKnee"])
    testCase["samplingRate"] = int(testCase["samplingRate"])
    testCase["frequency"] = float(testCase["frequency"])
    testCase["amplitude"] = float(testCase["amplitude"])
...
def gen_test_case_header_file(testCase):
    ...
    # Create an instance of AUDIOLIB_softClip
    softClip_processor = AUDIOLIB_softClip(
        dType=dType,
    ...
    in0 = np.zeros((inChannels, inSamples)).astype("float32")
    if testCase["testType"] == "STATIC":
        ...
        for i in range(inChannels):
            in0[i * inSamples : (i + 1) * inSamples] = generate_sine(
                inSamples, sampling_rate, frequency, amplitude
            )

    # Execute the softClip algorithm using the class
    out = softClip_processor.exec(in0)
    if plotSignals == "TRUE":
        file_io.plot_Signals(in0, out, inSamples, inChannels, testId)
...
testCasesCsvFile = "../AUDIOLIB_softClip/test_cases_list.csv"
...
idatFileName = "../../AUDIOLIB_softClip/AUDIOLIB_softClip_idat.c"
