...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "AUDIOLIB_gain_idat.h"'
    ...
    idatFile.write("static AUDIOLIB_gain_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    # Create an instance of AUDIOLIB_gain
    gain_processor = AUDIOLIB_gain(
        dType=dType, inChannels=inChannels, inSamples=inSamples
    )
    ...
    if dType[0] == "i" or dType[0] == "u":
        ...
        in1 = np.random.randint(low=minVal, high=maxVal, size=(1)).astype("int")
        out = gain_processor.exec(in0, in1[0])
    elif dType[0] == "f":
        in0 = np.random.uniform(
            low=minVal, high=maxVal, size=(inChannels, inSamples)
        ).astype("float32")
        in1 = np.random.uniform(low=minVal, high=maxVal, size=1).astype("float32")
        out = gain_processor.exec(in0, in1[0])
    elif dType[0] == "d":
        in0 = np.random.uniform(
            low=minVal, high=maxVal, size=(inChannels, inSamples)
        ).astype("double")
        in1 = np.random.uniform(low=minVal, high=maxVal, size=1).astype("double")
        out = gain_processor.exec(in0, in1[0])
...
testCasesCsvFile = "../AUDIOLIB_gain/test_cases_list.csv"
...
idatFileName = "../../AUDIOLIB_gain/AUDIOLIB_gain_idat.c"
