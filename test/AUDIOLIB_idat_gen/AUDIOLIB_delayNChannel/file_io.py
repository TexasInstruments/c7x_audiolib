...
def write_header_file(
    in0, in1, output_buffer, updated_state_buffer, testCase, headerFileName, delaySize
):
    testId = testCase["ID"]
    ...
    outputDir = "../../AUDIOLIB_delayNChannel/test_data/"
    ...
    with open(outputDir + headerFileName, "a") as f:
        write_attributes(
            f,
            attributesIn1,
            "uint32_t" + " " + "staticDelaySizeCase" + str(testId) + "[]=\n",
        )
        for dimY in range(0, numChannels):
