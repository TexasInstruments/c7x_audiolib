...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "AUDIOLIB_mute_idat.h"'
    ...
    idatFile.write("static AUDIOLIB_mute_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    """Convert str to int/float for necessary parameters.
    ...
    testCase["isMuted"] = float(testCase["isMuted"])
    ...
    # Set initialGain based on isMuted
    testCase["initialGain"] = 1.0 if testCase["isMuted"] == 1 else 0.0
    return testCase
...
def gen_test_case_header_file(testCase):
    ...
    # Create an instance of AUDIOLIB_mute
    mute_processor = AUDIOLIB_mute(
    ...
    # Execute the mute algorithm using the class
    out_list, gain_list = mute_processor.exec(pInLocal)

    # Reshape output for plotting and WAV file generation
    pOutLocal = np.array(out_list).reshape(numExecReps, samplesPerFrame)
    pCurrentGain = np.array(gain_list)
...
testCasesCsvFile = "../AUDIOLIB_mute/test_cases_list.csv"
...
idatFileName = "../../AUDIOLIB_mute/AUDIOLIB_mute_idat.c"
