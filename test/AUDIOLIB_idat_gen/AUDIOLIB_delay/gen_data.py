...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "AUDIOLIB_delay_idat.h"'
    ...
    idatFile.write("static AUDIOLIB_delay_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["delaySize"] = int(testCase["delaySize"])
    testCase["numExecReps"] = int(testCase["numExecRep"])
    return testCase
...
def gen_test_case_header_file(testCase):
    ...
    # Create an instance of AUDIOLIB_delay
    delay_processor = AUDIOLIB_delay(
        dType=dType,
        mode=mode,
        delaySize=delaySize,
    ...
    # Store the original input buffer for header file generation
    inputBuffTemp = inputBuff.copy()

    # Execute the delay algorithm using the class
    outBuff, delayBuff = delay_processor.exec(inputBuff)

    # Calculate delayBuffSize based on mode for header file generation
    if mode == 0:
        delayBuffSize = numSamples + delaySize
    else:  # mode == 1
        if (numSamples + delaySize) < 512:
            delayBuffSize = 512
        else:
            delayBuffSize = 2 ** int(np.ceil(np.log2(numSamples + delaySize)))

    headerFileName = "staticRefCase" + str(testId) + ".h"
    file_io.write_header_file(
        inputBuffTemp, outBuff, delayBuff, testCase, headerFileName, delayBuffSize
    )
    return [np.min(outBuff), np.max(outBuff)]


# CSV file with paratmeters for all test cases
testCasesCsvFile = "../AUDIOLIB_delay/test_cases_list.csv"
testIdList = []  # list of test IDs that we want to generate test data
testIdList += range(0, 150)
# testIdList = [18]
...
idatFileName = "../../AUDIOLIB_delay/AUDIOLIB_delay_idat.c"
