...
int AUDIOLIB_concat_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_concat_testParams_t *prm;
   AUDIOLIB_concat_testParams_t  currPrm;
   AUDIOLIB_concat_getTestParams(&prm, &test_cases);

   AUDIOLIB_concat_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 0\n");

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 1\n");

   TI_profile_init("AUDIOLIB_concat");
   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_concat.csv", "w+");
   fprintf(fpOutputCSV, "Test ID, Bit Width, isInterleave, inSamples, inOutputChannels, "
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (isInterleave == 1) {
            ...
            bufParamsOut.dim_y     = totalOutChannels;
            bufParamsOut.stride_y  = bufParamsOut.dim_x * AUDIOLIB_sizeof(bufParamsOut.data_type);
         }
         ...
         for (i = 0; i < currPrm.numInputs; i++) {
            uint32_t inp0SizeBytes = bufParamsInX[i].dim_y * bufParamsInX[i].stride_y;
         ...
         if (pInX && pOut && pOutCn) {
            ...
            status_init = AUDIOLIB_concat_init_checkParams(handle, bufParamsInX, &bufParamsOut, &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 1 status_init %d\n", status_init);
            if (status_init == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               AUDIOLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
               status_init           = AUDIOLIB_concat_init(handle, bufParamsInX, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 2 status_init %d\n", status_init);

            status_opt = AUDIOLIB_concat_exec_checkParams(handle, (const void **) pInX, pOut);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 3 status_opt %d\n", status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_concat_exec(handle, pInX, pOut);
               AUDIOLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 4 status_opt %d\n", status_opt);
            // initialize the kernel to use the natural C variant
            AUDIOLIB_concat_init(handle, bufParamsInX, &bufParamsOut, &kerInitArgs);

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_concat_exec(handle, pInX, pOutCn);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0,
                                  "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 8 status_nat_vs_opt %d status_ref_vs_opt "
                                  "%d currentTestFail "
                                  "%d\n",
                                  status_nat_vs_opt, status_ref_vs_opt, currentTestFail);

            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_concat_d CP 8 fail %d\n", fail);
            ...
            sprintf(desc, "%s generated input | inSamples = %d, inOutputChannels = %d", testPatternString,
                    currPrm.inSamples, totalOutChannels);
            // write to CSV, must happen prior to write to screen because
            // TI_profile_formula_add clears values in counters
            AUDIOLIB_concat_perfEst(handle, &archCycles, &estCycles);
            fprintf(fpOutputCSV, "%d, %d, %d, %d, %d, %d ,%ld, %d\n", currPrm.testID,
                    AUDIOLIB_sizeof(currPrm.dataType) * 8, isInterleave, currPrm.inSamples, totalOutChannels,
                    pProfile[3 * tpi + 1], estCycles, !currentTestFail);

            TI_profile_add_test(testNum++, (currPrm.inSamples * totalOutChannels), archCycles, estCycles,
            ...
            // clear the counters between runs; normally handled by TI_profile_concat_test
...
#endif
   {
      return AUDIOLIB_concat_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   memset(&kerInitArgs, 0, sizeof(AUDIOLIB_concat_InitArgs));
   ...
   while (testNum <= 1010) {
      ...
      case 1000:
         ...
         status_nat = AUDIOLIB_concat_init_checkParams(NULL, bufParamsInTemp, &bufParamsOutTemp, &kerInitArgs);

         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt = AUDIOLIB_concat_init_checkParams(NULL, bufParamsInTemp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat = AUDIOLIB_concat_init_checkParams(handle, bufParamsInTemp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1005:
         ...
         bufParamsInTemp[1].dim_x = inSamples;
         bufParamsInTemp[1].dim_y = 8; // != inChannels (16)
      ...
      case 1006:
         ...
         bufParamsInTemp[0].dim_y = inChannels;
         bufParamsInTemp[1].dim_x = inSamples;
         bufParamsInTemp[1].dim_y = inChannels;
         bufParamsOutTemp.dim_x   = inSamples;
   ...
   int32_t handleSize_LDRA = AUDIOLIB_concat_getHandleSize(&kerInitArgs);
...
/* Main call for concat test projects */
