...
int AUDIOLIB_typeConversion_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_typeConversion_testParams_t *prm;
   AUDIOLIB_typeConversion_testParams_t  currPrm;
   AUDIOLIB_typeConversion_getTestParams(&prm, &test_cases);

   AUDIOLIB_typeConversion_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 0\n");

   int32_t               handleSize = AUDIOLIB_typeConversion_getHandleSize(&kerInitArgs);
   AUDIOLIB_kernelHandle handle     = malloc(handleSize);

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 1\n");

   TI_profile_init("AUDIOLIB_typeConversion");
   // file IO for EVM benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_typeConversion.csv", "w+");
   fprintf(fpOutputCSV, "Test ID, Bit Width, inSamples, inChannels, "
                        "estCycles,EVM cycles, Pass/Fail\n");

   for (tpi = 0; tpi < test_cases; tpi++) {
      numReps             = prm[tpi].numReps;
      testNum             = prm[tpi].testID;
      currPrm             = prm[tpi];
      kerInitArgs.testQ31 = currPrm.testQ31;
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         bufParamsIn0.data_type = currPrm.inDataType;
         bufParamsIn0.dim_x     = currPrm.inSamples;
         bufParamsIn0.dim_y     = currPrm.inChannels;
         bufParamsIn0.stride_y  = bufParamsIn0.dim_x * AUDIOLIB_sizeof(currPrm.inDataType);

         bufParamsOut.data_type = currPrm.outDataType;
         bufParamsOut.dim_x     = currPrm.inSamples;
         bufParamsOut.dim_y     = currPrm.inChannels;
         bufParamsOut.stride_y  = bufParamsOut.dim_x * AUDIOLIB_sizeof(currPrm.outDataType);
         ...
         if (pIn0 && pOut && pOutCn) {
            ...
            if (currPrm.inDataType == AUDIOLIB_FLOAT32 || currPrm.inDataType == AUDIOLIB_FLOAT64) {

               TI_fillBuffer_float(prm[tpi].testPattern, 0, pIn0, prm[tpi].staticIn0, bufParamsIn0.dim_x,
                                   bufParamsIn0.dim_y, bufParamsIn0.stride_y, AUDIOLIB_sizeof(currPrm.inDataType),
                                   testPatternString);
            }
            else {
            ...
            status_init = AUDIOLIB_typeConversion_init_checkParams(handle, &bufParamsIn0, &bufParamsOut, &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 1 status_init %d\n",
                                  status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               status_init           = AUDIOLIB_typeConversion_init(handle, &bufParamsIn0, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 2 status_init %d\n",
                                  status_init);

            status_opt = AUDIOLIB_typeConversion_exec_checkParams(handle, pIn0, pOut);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 3 status_opt %d\n", status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_typeConversion_exec(handle, pIn0, pOut);
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 4 status_opt %d\n", status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_typeConversion_init(handle, &bufParamsIn0, &bufParamsOut, &kerInitArgs);

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_typeConversion_exec(handle, pIn0, pOutCn);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.outDataType == AUDIOLIB_FLOAT32 || currPrm.outDataType == AUDIOLIB_FLOAT64) {
               status_nat_vs_opt = TI_compare_mem_2D_float((void *) pOut, (void *) pOutCn, 0, (double) powf(2, -10),
                                                           bufParamsOut.dim_x, bufParamsOut.dim_y,
                                                           bufParamsOut.stride_y, AUDIOLIB_sizeof(currPrm.outDataType));
            }
            else {
               status_nat_vs_opt =
                   TI_compare_mem_2D((void *) pOut, (void *) pOutCn, 0x1, 0, bufParamsOut.dim_x, bufParamsOut.dim_y,
                                     bufParamsOut.stride_y, AUDIOLIB_sizeof(currPrm.outDataType));
            }
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
            ...
            if (currPrm.staticOut != NULL) {
               ...
               if (currPrm.outDataType == AUDIOLIB_FLOAT32 || currPrm.outDataType == AUDIOLIB_FLOAT64) {
                  ...
                  status_ref_vs_opt = TI_compare_mem_2DDualStride_float(
                      ...
                      bufParamsOut.dim_x * AUDIOLIB_sizeof(currPrm.outDataType), AUDIOLIB_sizeof(currPrm.outDataType));
               }
               else {
                  status_ref_vs_opt = TI_compare_mem_2DDualStride(
                      (void *) pOut, currPrm.staticOut, 0x1, 0, bufParamsOut.dim_x, bufParamsOut.dim_y,
                      bufParamsOut.stride_y, bufParamsOut.dim_x * AUDIOLIB_sizeof(currPrm.outDataType),
                      AUDIOLIB_sizeof(currPrm.outDataType));
               }
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0,
                "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 8 status_nat_vs_opt %d status_ref_vs_opt "
                "%d currentTestFail "
                "%d\n",
                status_nat_vs_opt, status_ref_vs_opt, currentTestFail);

            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_typeConversion_d CP 8 fail %d\n", fail);
            ...
            sprintf(desc, "%s generated input | inSamples = %d, inChannels = %d", testPatternString, currPrm.inSamples,
                    currPrm.inChannels);
            // Get performance estimate (needed for TI_profile_add_test)
            AUDIOLIB_typeConversion_perfEst(handle, &archCycles, &estCycles, currPrm.inDataType, currPrm.outDataType);
            ...
            // clear the counters between runs; normally handled by TI_profile_typeConversion_test
      ...
      // Write to CSV once per test case (after all repetitions)
      // Uses final values from last repetition
      fprintf(fpOutputCSV, "%d, %d, %d, %d,%ld, %d , %d\n", currPrm.testID, AUDIOLIB_sizeof(currPrm.outDataType) * 8,
              currPrm.inSamples, currPrm.inChannels, estCycles, pProfile[3 * tpi + 1], !currentTestFail);
   } // end idat test cases
...
int test_main(uint32_t *pProfile)
{
   ...
   if (TI_cache_init()) {
      TI_memError("AUDIOLIB_typeConversion");
      return 1;
   }
   else
#else
...
#endif
   {
      return AUDIOLIB_typeConversion_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1004) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat =
             AUDIOLIB_typeConversion_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt =
             AUDIOLIB_typeConversion_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat =
             AUDIOLIB_typeConversion_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt =
             AUDIOLIB_typeConversion_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         /* Valid type pair but mismatched dim_x -> INVALID_DIMENSION
          * (covers the first operand of the dimension-consistency check). */
         bufParamsIn0Temp.data_type = AUDIOLIB_INT16;
         bufParamsOutTemp.data_type = AUDIOLIB_FLOAT32;

         bufParamsIn0Temp.dim_x = bufParamsIn0.dim_x;
         bufParamsOutTemp.dim_x = bufParamsIn0.dim_x + 1;

         bufParamsIn0Temp.dim_y = bufParamsIn0.dim_y;
         bufParamsOutTemp.dim_y = bufParamsIn0.dim_y;
         ...
         break;
      case 1004:
         /* Valid type pair, matching dim_x but mismatched dim_y -> INVALID_DIMENSION
          * (covers the second operand of the dimension-consistency check). */
         bufParamsIn0Temp.data_type = AUDIOLIB_INT16;
         bufParamsOutTemp.data_type = AUDIOLIB_FLOAT32;

         bufParamsIn0Temp.dim_x = bufParamsIn0.dim_x;
         bufParamsOutTemp.dim_x = bufParamsIn0.dim_x;

         bufParamsIn0Temp.dim_y = bufParamsIn0.dim_y;
         bufParamsOutTemp.dim_y = bufParamsIn0.dim_y + 1;
   ...
   /* For every call of AUDIOLIB_typeConversion_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_typeConversion_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_typeConversion_getHandleSize(&kerInitArgs);
...
/* Main call for intypeConversionidual test projects */
