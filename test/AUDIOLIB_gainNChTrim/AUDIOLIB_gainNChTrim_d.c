...
int AUDIOLIB_gainNChTrim_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   AUDIOLIB_bufParams1D_t bufParamsIn1, bufParamsIn2;
   ...
   AUDIOLIB_gainNChTrim_testParams_t *prm;
   AUDIOLIB_gainNChTrim_testParams_t  currPrm;
   AUDIOLIB_gainNChTrim_getTestParams(&prm, &test_cases);

   AUDIOLIB_gainNChTrim_InitArgs kerInitArgs;

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 0\n");

   int32_t               handleSize = AUDIOLIB_gainNChTrim_getHandleSize(&kerInitArgs);
   AUDIOLIB_kernelHandle handle     = malloc(handleSize);

   AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 1\n");

   TI_profile_init("AUDIOLIB_gainNChTrim");
   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("AUDIOLIB_gainNChTrim.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         bufParamsIn2.data_type = currPrm.dataType;
         ...
         uint32_t inp2SizeBytes = bufParamsIn2.dim_x * dataWidth;
         ...
         void *pIn2 = (void *) TI_memalign(AUDIOLIB_L2DATA_ALIGNMENT, inp2SizeBytes);
         ...
         if (pIn0 && pIn1 && pIn2 && pOut && pOutCn) {
            ...
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
               ...
               TI_fillBuffer_float(prm[tpi].testPattern, 0, pIn2, prm[tpi].staticIn2, bufParamsIn2.dim_x, 1, 0,
                                   AUDIOLIB_sizeof(currPrm.dataType), testPatternString);
            }
            else {
            ...
            status_init = AUDIOLIB_gainNChTrim_init_checkParams(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut,
                                                                &kerInitArgs);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 1 status_init %d\n", status_init);
            ...
            if (status_init == AUDIOLIB_SUCCESS) {
               ...
               status_init =
                   AUDIOLIB_gainNChTrim_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);
               AUDIOLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 2 status_init %d\n", status_init);

            status_opt = AUDIOLIB_gainNChTrim_exec_checkParams(handle, pIn0, pIn1, pIn2, pOut);

            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 3 status_opt %d\n", status_opt);

            if (status_opt == AUDIOLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               AUDIOLIB_asm(" MARK 2");
               status_opt = AUDIOLIB_gainNChTrim_exec(handle, pIn0, pIn1, pIn2, pOut);
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 4 status_opt %d\n", status_opt);

            // initialize the kernel to use the natural C variant
            AUDIOLIB_gainNChTrim_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            AUDIOLIB_asm(" MARK 8");
            status_nat = AUDIOLIB_gainNChTrim_exec(handle, pIn0, pIn1, pIn2, pOutCn);
            AUDIOLIB_asm(" MARK 9");
            TI_profile_stop();

#if AUDIOLIB_DEBUGPRINT
            printf("AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == AUDIOLIB_FLOAT32 || currPrm.dataType == AUDIOLIB_FLOAT64) {
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(
                0,
                "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 8 status_nat_vs_opt %d status_ref_vs_opt "
                "%d currentTestFail "
                "%d\n",
                status_nat_vs_opt, status_ref_vs_opt, currentTestFail);

            AUDIOLIB_DEBUGPRINTFN(
                0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 8 status_init %d status_opt %d status_nat %d\n",
            ...
            AUDIOLIB_DEBUGPRINTFN(0, "AUDIOLIB_DEBUGPRINT  AUDIOLIB_gainNChTrim_d CP 8 fail %d\n", fail);
            ...
            AUDIOLIB_gainNChTrim_perfEst(handle, &archCycles, &estCycles, currPrm.dataType, bufParamsIn0.dim_x,
                                         bufParamsIn0.stride_y, bufParamsOut.stride_y);
            ...
            // clear the counters between runs; normally handled by TI_profile_gainNChTrim_test
...
int test_main(uint32_t *pProfile)
{
   ...
   if (TI_cache_init()) {
      TI_memError("AUDIOLIB_gainNChTrim");
      return 1;
   }
   else
#else
...
#endif
   {
      return AUDIOLIB_gainNChTrim_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1007) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_gainNChTrim_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_gainNChTrim_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_NATC;
         status_nat            = AUDIOLIB_gainNChTrim_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_opt            = AUDIOLIB_gainNChTrim_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                       &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         /* Valid types but gain length != channel count -> INVALID_DIMENSION. */
         bufParamsIn0Temp.data_type = bufParamsIn0.data_type;
         bufParamsIn1Temp.data_type = bufParamsIn1.data_type;
         bufParamsOutTemp.data_type = bufParamsOut.data_type;

         bufParamsIn0Temp.dim_x = bufParamsIn0.dim_x;
         bufParamsIn1Temp.dim_x = bufParamsIn0.dim_x + 1; /* Invalid */
         bufParamsOutTemp.dim_x = bufParamsOut.dim_x;

         bufParamsIn0Temp.dim_y = bufParamsIn0.dim_y;
         bufParamsOutTemp.dim_y = bufParamsOut.dim_y;

         kerInitArgs.isInterleave = 0; /* Ensures numChannels = dim_y */
         kerInitArgs.funcStyle    = AUDIOLIB_FUNCTION_NATC;
         status_nat               = AUDIOLIB_gainNChTrim_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                          &bufParamsOutTemp, &kerInitArgs);
         ...
         break;
      case 1004:
         /* Valid types and gain length, but out.dim_x != in.dim_x -> INVALID_DIMENSION
          * (covers the first operand of the input/output dimension check). */
         bufParamsIn0Temp.data_type = bufParamsIn0.data_type;
         bufParamsIn1Temp.data_type = bufParamsIn1.data_type;
         bufParamsOutTemp.data_type = bufParamsOut.data_type;
         ...
         bufParamsOutTemp.dim_x = bufParamsIn0.dim_x + 1; /* Invalid: != in.dim_x */
         ...
         break;
      case 1005:
         /* Valid types and gain length, in.dim_x == out.dim_x but out.dim_y != in.dim_y ->
          * INVALID_DIMENSION (covers the second operand of the input/output dimension check). */
         bufParamsIn0Temp.data_type = bufParamsIn0.data_type;
         bufParamsIn1Temp.data_type = bufParamsIn1.data_type;
         bufParamsOutTemp.data_type = bufParamsOut.data_type;
         ...
         bufParamsIn0Temp.dim_y = bufParamsIn0.dim_y;
         bufParamsOutTemp.dim_y = bufParamsIn0.dim_y + 1; /* Invalid: != in.dim_y */
         ...
         break;
      case 1006:
         /* init() with NULL handle -> NULL_POINTER (covers the init null-check). */
         bufParamsIn0Temp.data_type = bufParamsIn0.data_type;
         bufParamsIn1Temp.data_type = bufParamsIn1.data_type;
         bufParamsOutTemp.data_type = bufParamsOut.data_type;
         ...
         kerInitArgs.funcStyle    = AUDIOLIB_FUNCTION_OPTIMIZED;
         status_nat =
             AUDIOLIB_gainNChTrim_init(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, &kerInitArgs);
         currentTestFail = (status_nat != AUDIOLIB_ERR_NULL_POINTER);
         break;
      case 1007:
         /* exec_checkParams() with NULL handle -> NULL_POINTER (covers the handle null-check). */
         status_opt      = AUDIOLIB_gainNChTrim_exec_checkParams(NULL, pIn0, pIn1, pIn1, pOut);
   ...
   /* For every call of AUDIOLIB_gainNChTrim_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling AUDIOLIB_gainNChTrim_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = AUDIOLIB_gainNChTrim_getHandleSize(&kerInitArgs);
...
/* Main call for ingainNChTrimidual test projects */
