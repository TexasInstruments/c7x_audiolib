...
#ifndef AUDIOLIB_SSRC_IXX_IXX_OXX_IDAT_H

#define AUDIOLIB_SSRC_IXX_IXX_OXX_IDAT_H
...
typedef struct {
   ...
   uint32_t enableMMA; // 1 -> enable MMA, 0 -> Disable MMA or if the SoC does not support MMA
   uint32_t dataFormat;
   uint32_t outputDataLocation; // 0 -> HEAP (probably L2SRAM), 1 -> MSMC
   uint32_t numReps;
   uint32_t testCategory;
   uint32_t testID;
...
#endif /* define AUDIOLIB_SSRC_IXX_IXX_OXX_IDAT_H */
