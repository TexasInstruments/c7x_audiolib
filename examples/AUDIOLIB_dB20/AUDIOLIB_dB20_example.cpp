...
int main(void)
{
   ...
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_dB20_init_checkParams(handle, &bufParamsIn0, &bufParamsOut, &kerInitArgs);

   // init
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_dB20_init(handle, &bufParamsIn0, &bufParamsOut, &kerInitArgs);

   // exec checkparams
   if (status == AUDIOLIB_SUCCESS)
      AUDIOLIB_dB20_exec_checkParams(handle, in0, out);

   // exec
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_dB20_exec(handle, in0, out);
   ...
   // exec checkparams
   if (status == AUDIOLIB_SUCCESS)
      AUDIOLIB_dB20_exec_checkParams(handle, in0D, out);

   // exec
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_dB20_exec(handle, in0D, outD);
