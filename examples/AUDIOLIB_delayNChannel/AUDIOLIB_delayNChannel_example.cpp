...
int main(void)
{
   ...
   kerInitArgs.delaySize[0] = 8;
   kerInitArgs.delaySize[1] = 16;

   status = AUDIOLIB_SUCCESS;

   // init checkparams
   //    status = AUDIOLIB_delayNChannel_init_checkParams(handle, &bufParamsIn, &bufParamsDelay, &bufParamsOut,
   //    &kerInitArgs);

   // init
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_delayNChannel_init(handle, &bufParamsIn, &bufParamsDelay, &bufParamsOut, &kerInitArgs);

   // exec checkparams

   // exec
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_delayNChannel_exec(handle, in, delay, out, scratch);
