...
int main(void)
{
  ...
  // Setup input and output buffers for single- and double-precision datatypes
   ...
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_gainNCh_init_checkParams(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);

   // init
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_gainNCh_init(handle, &bufParamsIn0, &bufParamsIn1, &bufParamsOut, &kerInitArgs);

   // exec checkparams
   if (status == AUDIOLIB_SUCCESS)
      AUDIOLIB_gainNCh_exec_checkParams(handle, in0, in1, out);

   // exec
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_gainNCh_exec(handle, in0, in1, out);
   ...
   // exec
   if (status == AUDIOLIB_SUCCESS)
      status = AUDIOLIB_gainNCh_exec(handle, in0D, in1D, outD);
