...
#ifndef AUDIOLIB_DB10_IXX_IXX_OXX_PRIV_H_
#define AUDIOLIB_DB10_IXX_IXX_OXX_PRIV_H_

#include "../common/AUDIOLIB_utility.h"
#include "AUDIOLIB_dB10.h"

/**
 * @brief Macro to define the size of bufPblock array of
 *        @ref AUDIOLIB_dB10_PrivArgs structure.
 *
 */

#define AUDIOLIB_DB10_IXX_IXX_OXX_PBLOCK_SIZE (2 * SE_PARAM_SIZE + 1 * AUDIOLIB_PARAM_SIZE + 2 * sizeof(int32_t))
...
#define FLT_MAX_DB10 3.402823466E+38
...
#define DBL_MAX_DB10 1.7976931348623157e+308
...
#define CONST_DOUBLE_MIN_DB10 1.843542739450977e+19
...
#define CONST_MAX_DB10 308.254715974092

/**
 * @brief Pre-x10 dB floor returned for dB10(x <= 0): log10(FLT_MIN).
 *        After the x10 scaling this is ~ -379.3 dB, a large-negative
 *        "silence" level continuous with the x -> 0+ limit. Was 4286578688
 *        (the integer reading of the -Inf bit pattern 0xFF800000), which
 *        wrongly produced +4.29e10 dB. See XLIB-1033.
 */

#define CONST_FLOAT_MIN_DB10 (-37.9297794537f)

/**
 *  @brief This is a function pointer type that conforms to the
 *         declaration of @ref AUDIOLIB_dB10_exec_ci
 *         and @ref AUDIOLIB_dB10_exec_cn.
 */

/*
 * @brief Executes the vectorized C7x implementation of the dB10 kernel.
 ...
 */
typedef AUDIOLIB_STATUS (*pFxnAUDIOLIB_dB10_exec)(AUDIOLIB_kernelHandle handle,
                                                  void *restrict pIn,
                                                  void *restrict pOut);
 ...
 *         to the declaration of @ref AUDIOLIB_dB10_init.
 *
 * @details This function determines the configuration for the streaming engine
 *          and MMA hardware resources based on the function call parameters,
 *          and the configuration is saved in bufPBlock array. In the kernel
 *          call sequence, @ref AUDIOLIB_dB10_exec_ci would be
 *          called later independently by the application. When
 *          @ref AUDIOLIB_dB10_exec_ci runs, it merely retrieves
 *          the configuration from the bufPBlock and uses it to set up the
 *          hardware resources. This arrangement is so that
 *          @ref AUDIOLIB_dB10_exec_ci does not lose cycles
...
extern AUDIOLIB_STATUS AUDIOLIB_dB10_init_ci(AUDIOLIB_kernelHandle         handle,
...
AUDIOLIB_dB10_vector_sp_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
#endif

/**
 * @brief Executes the dB10 vector split1 kernel for the specified data type.
 *
 * This function processes input data using the dB10 vector split1 algorithm and writes the result to the output buffer.
 *
...
/**
 * @brief Executes the split2 variant of the vectorized C7x dB10 kernel.
 *
 * @param handle   Active handle to the kernel
 * @param pIn      Pointer to input buffer
 * @param pOut     Pointer to output buffer
...
/**
 * @brief Executes the dB10 scaler kernel for 1x1 input/output using the specified data type.
 *
 * This function applies the dB10 scaling operation on the input data and writes the result to the output buffer.
 *
 ...
 */
template <typename dataType>
AUDIOLIB_STATUS AUDIOLIB_dB10_scaler_1x1_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);

/**
 * @brief Executes the scalar Mx1 C7x implementation of the dB10 kernel.
 *
 * @param handle   Active handle to the kernel
 * @param pIn      Pointer to input buffer
 * @param pOut     Pointer to output buffer
...
/**
 * @brief Executes the 1D vectorized C7x implementation of the dB10 scaler kernel.
...
template <typename dataType>
extern AUDIOLIB_STATUS
AUDIOLIB_dB10_scaler_sp_1xN_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);

/**
 * @brief Executes the 2D vectorized C7x implementation of the dB10 scaler kernel.
 *        Only available for __C7504__ target.
 *
 * @param handle   Active handle to the kernel
 * @param pIn      Pointer to input buffer
 ...
 *         to the declaration of @ref AUDIOLIB_dB10_exec.
 *
 * @details
 *
 *  @param [in]  handle      : Active handle to the kernel
...
template <typename dataType>
extern AUDIOLIB_STATUS AUDIOLIB_dB10_exec_cn(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
typedef struct {
    ...
    *         @ref AUDIOLIB_dB10_exec_cn and
    *         @ref AUDIOLIB_dB10_exec_ci.                        */
   pFxnAUDIOLIB_dB10_exec execute;
   ...
   uint8_t bufPblock[AUDIOLIB_DB10_IXX_IXX_OXX_PBLOCK_SIZE];
} AUDIOLIB_dB10_PrivArgs;

#endif /* AUDIOLIB_DB10_IXX_IXX_OXX_PRIV_H_ */
