...
#ifndef AUDIOLIB_TABLELOOKUP_IXX_IXX_OXX_PRIV_H_
#define AUDIOLIB_TABLELOOKUP_IXX_IXX_OXX_PRIV_H_

#include "../common/AUDIOLIB_utility.h"
#include "AUDIOLIB_tableLookup.h"

/*!
 * @brief Macro to define the size of bufPblock array of
 *        @ref AUDIOLIB_tableLookup_PrivArgs structure.
 *
 */
#define AUDIOLIB_TABLELOOKUP_IXX_IXX_OXX_PBLOCK_SIZE (1 * SE_PARAM_SIZE + 2 * SA_PARAM_SIZE)
 ...
 *         declaration of @ref AUDIOLIB_tableLookup_exec_ci
 *         and @ref AUDIOLIB_tableLookup_exec_cn.
 */
typedef AUDIOLIB_STATUS (*pFxnAUDIOLIB_tableLookup_exec)(AUDIOLIB_kernelHandle handle,
                                                         void *restrict pIn0,
                                                         void *restrict pIn1,
                                                         void *restrict pOut);
 ...
 *         to the declaration of @ref AUDIOLIB_tableLookup_init.
 *
 * @details This function determines the configuration for the streaming engine
 *          and MMA hardware resources based on the function call parameters,
 *          and the configuration is saved in bufPBlock array. In the kernel
 *          call sequence, @ref AUDIOLIB_tableLookup_exec_ci would be
 *          called later independently by the application. When
 *          @ref AUDIOLIB_tableLookup_exec_ci runs, it merely retrieves
 *          the configuration from the bufPBlock and uses it to set up the
 *          hardware resources. This arrangement is so that
 *          @ref AUDIOLIB_tableLookup_exec_ci does not lose cycles
 ...
 *         implementation of the kernel. The function declaration conforms
 *         to the declaration of @ref AUDIOLIB_tableLookup_exec.
 *
 ...
 *          a stream tableLookupress generator.
 *
 *  @param [in]  handle      : Active handle to the kernel
 ...
 * declaration conforms to the declaration of @ref AUDIOLIB_tableLookup_exec.
 *
 *  @param [in]  handle      : Active handle to the kernel
 *  @param [in]  pIn0        : Pointer to buffer holding the src buffer
 *  @param [in]  pIn1        : Pointer to buffer holding the table buffer
 ...
 */
template <typename dataType>
extern AUDIOLIB_STATUS AUDIOLIB_tableLookup_exec_cn(AUDIOLIB_kernelHandle handle,
                                                    void *restrict pIn0,
                                                    void *restrict pIn1,
                                                    void *restrict pOut);
...
typedef struct {
    ...
    *         @ref AUDIOLIB_tableLookup_exec_cn and
    *         @ref AUDIOLIB_tableLookup_exec_ci.                        */
   pFxnAUDIOLIB_tableLookup_exec execute;
   ...
   AUDIOLIB_tableLookup_InitArgs initArgs;     /**< Structure holding initialization parameters  */
   /*! @brief bufPblock array to store SE/SA template */
   uint8_t bufPblock[AUDIOLIB_TABLELOOKUP_IXX_IXX_OXX_PBLOCK_SIZE]; /*< Array to hold SE/SA template */
} AUDIOLIB_tableLookup_PrivArgs;
