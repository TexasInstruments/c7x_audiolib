...
#include "AUDIOLIB_undB10_priv.h"
...
{
   AUDIOLIB_undB10_PrivArgs *pKerPrivArgs = (AUDIOLIB_undB10_PrivArgs *) handle;
   ...
   uint64_t undB10StartupCycles   = 0;
   uint64_t undB10TeardownCycles  = 0;
   uint64_t undB10OverheadCycles  = 0;
   uint32_t nVecs                 = 0;
   uint64_t undB10OperationCycles = 0;
...
#if defined(__C7524__)
   {
      if (dataType == AUDIOLIB_FLOAT32) {
         if (numSamples == 1) {
            if (numChannels == 1) {
               undB10OperationCycles = 55;
            }
            else {
               nVecs                 = numChannels * numSamples;
               undB10OperationCycles = (67 + nVecs * 9);
            }
         }
         else {
            undB10StartupCycles  = 25;
            undB10TeardownCycles = 3;

            nVecs                 = pKerPrivArgs->nVecs;
            undB10OperationCycles = (61 + nVecs * 14) + (8 + nVecs * 3);
         }
      }
   }
#elif defined(__C7504__)
   {
      if (dataType == AUDIOLIB_FLOAT32) {
         ...
         if (numSamples == 1) {
            ...
            undB10OperationCycles = (64 + nVecs * 9);
         }
         else {
            undB10OperationCycles = (73 + nVecs * 9);
         }
      }
   }
#endif

   undB10OverheadCycles = undB10StartupCycles + undB10TeardownCycles;
   *estCycles           = undB10OperationCycles + undB10OverheadCycles;
   *archCycles          = undB10OperationCycles;
}

template <typename dataType>
AUDIOLIB_STATUS AUDIOLIB_undB10_init_ci(AUDIOLIB_kernelHandle           handle,
...
template AUDIOLIB_STATUS AUDIOLIB_undB10_init_ci<float>(AUDIOLIB_kernelHandle           handle,
                                                        const AUDIOLIB_bufParams2D_t   *bufParamsIn,
                                                        const AUDIOLIB_bufParams2D_t   *bufParamsOut,
                                                        const AUDIOLIB_undB10_InitArgs *pKerInitArgs);

template <typename dataType> static inline dataType AUDIOLIB_undB10_scalar_ci(dataType a);

template <> inline float AUDIOLIB_undB10_scalar_ci(float a)
{
...
AUDIOLIB_undB10_scaler_sp_1xN_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (uint32_t idx = 0; idx < pKerPrivArgs->nVecs; idx++) {
      pDst[idx] = AUDIOLIB_undB10_scalar_ci<dataType>(pSrc[idx]);
   }
   return status;
}

template AUDIOLIB_STATUS
AUDIOLIB_undB10_scaler_sp_1xN_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
AUDIOLIB_undB10_scaler_sp_MxN_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (uint32_t i = 0; i < pKerPrivArgs->nVecs; i++) {
      pDst[outOffset + idx] = AUDIOLIB_undB10_scalar_ci<dataType>(pSrc[inOffset + idx]);
      idx++;
...
template AUDIOLIB_STATUS
AUDIOLIB_undB10_scaler_sp_MxN_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);

#elif defined(__C7524__)

template <typename dataType>
static inline void AUDIOLIB_undB10_cond(AUDIOLIB_kernelHandle handle,
...
template <typename dataType>
AUDIOLIB_STATUS AUDIOLIB_undB10_vector_sp_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   AUDIOLIB_undB10_PrivArgs *pKerPrivArgs = (AUDIOLIB_undB10_PrivArgs *) handle;
   AUDIOLIB_STATUS status = AUDIOLIB_SUCCESS;
   uint8_t *pBlock = pKerPrivArgs->bufPblock;
   ...
   /***********************************************************************/
   /* Create and assign values for constants employed on undB10 computation */
   /***********************************************************************/
   ...
   AUDIOLIB_undB10_cond(handle, &se0Params, &sa0Params, pSrc, pDst);

   return status;
}

template AUDIOLIB_STATUS
AUDIOLIB_undB10_vector_sp_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
#endif
...
AUDIOLIB_undB10_scaler_1x1_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   pDst[0] = AUDIOLIB_undB10_scalar_ci<dataType>(pSrc[0]);

   return status;
}

template AUDIOLIB_STATUS
AUDIOLIB_undB10_scaler_1x1_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
AUDIOLIB_undB10_scaler_Mx1_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (uint32_t idx = 0; idx < numChannel; idx++) {
      pDst[outOffset] = AUDIOLIB_undB10_scalar_ci<dataType>(pSrc[inOffset]);
      outOffset += outStride;
      inOffset += inStride;
   }
...
template AUDIOLIB_STATUS
AUDIOLIB_undB10_scaler_Mx1_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
