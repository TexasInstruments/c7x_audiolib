...
{
   int32_t privBufSize = sizeof(AUDIOLIB_undB10_PrivArgs);
   return privBufSize;
}
                                 ...
                                 const AUDIOLIB_undB10_InitArgs *pKerInitArgs)
{
   ...
   printf("Enter AUDIOLIB_undB10_init_checkParams\n");
#endif
   if (handle == NULL) {
...
#if AUDIOLIB_DEBUGPRINT
   printf("Enter AUDIOLIB_undB10_exec_checkParams\n");
#endif
                                     ...
                                     const AUDIOLIB_undB10_InitArgs *pKerInitArgs)
{
...
#if AUDIOLIB_DEBUGPRINT
   printf("AUDIOLIB_DEBUGPRINT Enter AUDIOLIB_undB10_init\n");
#endif
   ...
   if (pKerInitArgs->funcStyle == AUDIOLIB_FUNCTION_NATC) {
      if (bufParamsIn->data_type == AUDIOLIB_FLOAT32) {
         pKerPrivArgs->execute = AUDIOLIB_undB10_exec_cn<float>;
      }
      else {
         status = AUDIOLIB_ERR_INVALID_TYPE;
      }
      ...
      if (bufParamsIn->data_type == AUDIOLIB_FLOAT32) {
         ...
         if (bufParamsIn->dim_x == 1) {
            if (bufParamsIn->dim_y > 1) {
               pKerPrivArgs->execute = AUDIOLIB_undB10_scaler_Mx1_exec_ci<float>;
            }
            else {
               pKerPrivArgs->execute = AUDIOLIB_undB10_scaler_1x1_exec_ci<float>;
            }
         }
         else {
#if defined(__C7524__)
            pKerPrivArgs->execute = AUDIOLIB_undB10_vector_sp_exec_ci<float>;
#elif defined(__C7504__)
            if (pKerPrivArgs->dim_x > 1 && pKerPrivArgs->dim_y == 1) {
               pKerPrivArgs->execute = AUDIOLIB_undB10_scaler_sp_1xN_exec_ci<float>;
            }
               ...
               pKerPrivArgs->execute = AUDIOLIB_undB10_scaler_sp_MxN_exec_ci<float>;
            }
#endif
         }
         status = AUDIOLIB_undB10_init_ci<float>(handle, bufParamsIn, bufParamsOut, pKerInitArgs);
      }
      else {
         status = AUDIOLIB_ERR_INVALID_TYPE;
      }
...
AUDIOLIB_STATUS AUDIOLIB_undB10_exec(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   AUDIOLIB_STATUS status;

#if AUDIOLIB_DEBUGPRINT
   printf("AUDIOLIB_DEBUGPRINT Enter AUDIOLIB_undB10_exec\n");
#endif
