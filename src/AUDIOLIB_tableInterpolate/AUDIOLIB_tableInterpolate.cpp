...
{
   int32_t privBufSize = sizeof(AUDIOLIB_tableInterpolate_PrivArgs);
   return privBufSize;
}
                                           ...
                                           const AUDIOLIB_tableInterpolate_InitArgs *pKerInitArgs)
{
   ...
   AUDIOLIB_DEBUGPRINTFN(0, "Enter AUDIOLIB_tableInterpolate_init_checkParams \n");
                                                           ...
                                                           const void *restrict pOut)
{
   ...
   AUDIOLIB_DEBUGPRINTFN(0, "Enter AUDIOLIB_tableInterpolate_exec_checkParams \n");
                                               ...
                                               const AUDIOLIB_tableInterpolate_InitArgs *pKerInitArgs)
{
   ...
   AUDIOLIB_DEBUGPRINTFN(0, "Enter AUDIOLIB_tableInterpolate_init \n");
   ...
   if (pKerInitArgs->funcStyle == AUDIOLIB_FUNCTION_NATC) {
      if (bufParamsIn0->data_type == AUDIOLIB_FLOAT32) {
         pKerPrivArgs->execute = AUDIOLIB_tableInterpolate_exec_cn<float>;
      }
      else {
         status = AUDIOLIB_ERR_INVALID_TYPE;
      }
      ...
      if (pKerPrivArgs->tableSamples > pKerInitArgs->tableInterpolateSize) {
         if (bufParamsIn0->data_type == AUDIOLIB_FLOAT32) {
            status = AUDIOLIB_tableInterpolate_unroll_init_ci<float>(handle, bufParamsIn0, bufParamsIn1, bufParamsOut,
                                                                     pKerInitArgs);
            pKerPrivArgs->execute = AUDIOLIB_tableInterpolate_unroll_exec_ci<float>;
         }
         else {
            status = AUDIOLIB_ERR_INVALID_TYPE;
         }
         ...
         if (bufParamsIn0->data_type == AUDIOLIB_FLOAT32) {
            status = AUDIOLIB_tableInterpolate_init_ci<float>(handle, bufParamsIn0, bufParamsIn1, bufParamsOut,
                                                              pKerInitArgs);
            pKerPrivArgs->execute = AUDIOLIB_tableInterpolate_exec_ci<float>;
         }
         else {
            status = AUDIOLIB_ERR_INVALID_TYPE;
         }
                               ...
                               void *restrict pOut)
{
   ...
   AUDIOLIB_DEBUGPRINTFN(0, "Enter AUDIOLIB_tableInterpolate_exec \n");
