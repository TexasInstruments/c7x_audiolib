...
#ifndef AUDIOLIB_LUT_H_
#define AUDIOLIB_LUT_H_ 1
...
/**
 * \defgroup AUDIOLIB_LUT Lookup table utilities
...
template <typename vecType> static inline c7x::uint_vec AUDIOLIB_LUTReadUpperBits(vecType vecOffset)
{
   c7x::uint_vec vecOut;
...
template <typename vecType> static inline c7x::uint_vec AUDIOLIB_LUTReadLowerBits(vecType vecOffset)
{
   c7x::uint_vec vecOut;
...
static inline void AUDIOLIB_LUTInit()
{
   ...
   uint32_t AUDIOLIB_LUTLow[tableSize] = {
   ...
   uint64_t      *pData = (uint64_t *) &AUDIOLIB_LUTHigh[0];
   c7x::ulong_vec vData;
   ...
   // Read the elements as a quantum of unsigned 64-bits
   pData     = (uint64_t *) &AUDIOLIB_LUTLow[0];
   rowOffset = 0;
   ...
   for (uint32_t j = 0; j < (tableSize / rowIncrement); j++) {
      ...
      printf("\nUpper Bits\n");
      AUDIOLIB_printVec(AUDIOLIB_LUTReadUpperBits(indices1));
      AUDIOLIB_printVec(AUDIOLIB_LUTReadUpperBits(indices2));
      printf("\nLower Bits\n");
      AUDIOLIB_printVec(AUDIOLIB_LUTReadLowerBits(indices1));
      AUDIOLIB_printVec(AUDIOLIB_LUTReadLowerBits(indices2));
#endif
   }
}
