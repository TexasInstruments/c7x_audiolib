...
#ifndef AUDIOLIB_UNDB20_IXX_IXX_OXX_PRIV_H_
#define AUDIOLIB_UNDB20_IXX_IXX_OXX_PRIV_H_

#include "../common/AUDIOLIB_utility.h"
#include "AUDIOLIB_undB20.h"

/**
 * @brief Macro to define the size of bufPblock array of
 *        @ref AUDIOLIB_undB20_PrivArgs structure.
 *
 */

#define AUDIOLIB_UNDB20_IXX_IXX_OXX_PBLOCK_SIZE (2 * SE_PARAM_SIZE + 1 * AUDIOLIB_PARAM_SIZE + 2 * sizeof(int32_t))
/**
 *  @brief This is a function pointer type that conforms to the
 *         declaration of @ref AUDIOLIB_undB20_exec_ci
 *         and @ref AUDIOLIB_undB20_exec_cn.
 */
typedef AUDIOLIB_STATUS (*pFxnAUDIOLIB_undB20_exec)(AUDIOLIB_kernelHandle handle,
                                                    void *restrict pIn,
                                                    void *restrict pOut);
 ...
 *         to the declaration of @ref AUDIOLIB_undB20_init.
 *
 * @details This function determines the configuration for the streaming engine
 *          and MMA hardware resources based on the function call parameters,
 *          and the configuration is saved in bufPBlock array. In the kernel
 *          call sequence, @ref AUDIOLIB_undB20_exec_ci would be
 *          called later independently by the application. When
 *          @ref AUDIOLIB_undB20_exec_ci runs, it merely retrieves
 *          the configuration from the bufPBlock and uses it to set up the
 *          hardware resources. This arrangement is so that
 *          @ref AUDIOLIB_undB20_exec_ci does not lose cycles
...
extern AUDIOLIB_STATUS AUDIOLIB_undB20_init_ci(AUDIOLIB_kernelHandle           handle,
                                               const AUDIOLIB_bufParams2D_t   *bufParamsIn,
                                               const AUDIOLIB_bufParams2D_t   *bufParamsOut,
                                               const AUDIOLIB_undB20_InitArgs *pKerInitArgs);
#if defined(__C7524__)
/**
 * @brief Executes the vectorized C7x implementation of the undB20 kernel.
...
AUDIOLIB_undB20_vector_sp_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
#endif

/**
 * @brief Executes the undB20 scaler kernel for 1x1 input/output using the specified data type.
 *
 * This function applies the undB20 scaling operation on the input data and writes the result to the output buffer.
 *
...
/**
 * @brief Executes the scalar Mx1 C7x implementation of the undB20 kernel.
 *
 * @param handle   Active handle to the kernel
 * @param pIn      Pointer to input buffer
 * @param pOut     Pointer to output buffer
...
/**
 * @brief Executes the 1D vectorized C7x implementation of the undB20 scaler kernel.
...
/*
 * @brief Executes the 2D vectorized C7x implementation of the undB20 scaler kernel.
 *        Only available for __C7504__ target.
 *
 * @param handle   Active handle to the kernel
 * @param pIn      Pointer to input buffer
 ...
 */
AUDIOLIB_STATUS
AUDIOLIB_undB20_init_cn(AUDIOLIB_kernelHandle   handle,
 ...
 *         to the declaration of @ref AUDIOLIB_undB20_exec.
 *
 * @details
 *
 *  @param [in]  handle      : Active handle to the kernel
...
template <typename dataType>
extern AUDIOLIB_STATUS AUDIOLIB_undB20_exec_cn(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
typedef struct {
    ...
    *         @ref AUDIOLIB_undB20_exec_cn and
    *         @ref AUDIOLIB_undB20_exec_ci.                        */
   pFxnAUDIOLIB_undB20_exec execute;
   /** @brief Size of input buffer for different batches
    *         @ref AUDIOLIB_undB20_init that will be retrieved
    *         and used by @ref AUDIOLIB_undB20_exec                */
   ...
   uint8_t bufPblock[AUDIOLIB_UNDB20_IXX_IXX_OXX_PBLOCK_SIZE];
} AUDIOLIB_undB20_PrivArgs;

#endif /* AUDIOLIB_UNDB20_IXX_IXX_OXX_PRIV_H_ */
