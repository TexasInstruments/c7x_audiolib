...
#ifndef AUDIOLIB_DELAY_IXX_IXX_OXX_PRIV_H_
#define AUDIOLIB_DELAY_IXX_IXX_OXX_PRIV_H_

#include "../common/AUDIOLIB_utility.h"
#include "AUDIOLIB_delay.h"

/*!
 * @brief Macro to define the size of bufPblock array of
 *        @ref AUDIOLIB_delay_PrivArgs structure.
 *
 */

#define AUDIOLIB_DELAY_IXX_IXX_OXX_PBLOCK_SIZE (5 * SE_PARAM_SIZE + 5 * SA_PARAM_SIZE)

/*!
 *  @brief This is a function pointer type that conforms to the
 *         declaration of @ref AUDIOLIB_delay_exec_ci
 *         and @ref AUDIOLIB_delay_exec_cn.
 */
typedef AUDIOLIB_STATUS (*pFxnAUDIOLIB_delay_exec)(AUDIOLIB_kernelHandle handle,
                                                   void *restrict pIn,
 ...
 *         to the declaration of @ref AUDIOLIB_delay_init.
 *
 * @details This function determines the configuration for the streaming engine
 *          and MMA hardware resources based on the function call parameters,
 *          and the configuration is saved in bufPBlock array. In the kernel
 *          call sequence, @ref AUDIOLIB_delay_exec_ci would be
 *          called later independently by the application. When
 *          @ref AUDIOLIB_delay_exec_ci runs, it merely retrieves
 *          the configuration from the bufPBlock and uses it to set up the
 *          hardware resources. This arrangement is so that
 *          @ref AUDIOLIB_delay_exec_ci does not lose cycles
 ...
 *          and MMA hardware resources based on the function call parameters,
 *          and the configuration is saved in bufPBlock array. In the kernel
 *          call sequence, @ref AUDIOLIB_delay_interleave_exec_ci would be
 *          called later independently by the application. When
 *          @ref AUDIOLIB_delay_exec_ci runs, it merely retrieves
 *          the configuration from the bufPBlock and uses it to set up the
 *          hardware resources. This arrangement is so that
 *          @ref AUDIOLIB_delay_interleave_exec_ci does not lose cycles
 *          to determine the hardware configuration.
 *
 *  @param [in]  handle         :  Active handle to the kernel
 *  @param [in]  bufParamsIn    :  Pointer to the structure containing dimensional
 ...
 *  @param [in]  bufParamsDelay :  Pointer to the structure containing dimensional
 *                                 information of delay buffer (non-interleaved)
 *  @param [out] bufParamsOut   :  Pointer to the structure containing dimensional
 ...
 *         implementation of the kernel. The function declaration conforms
 *         to the declaration of @ref AUDIOLIB_delay_exec.
 *
 ...
 */
template <typename dataType>
extern AUDIOLIB_STATUS AUDIOLIB_delay_interleave_exec_cn(AUDIOLIB_kernelHandle handle,
                                                         void *restrict pIn,
                                                         void *restrict pDelay,
                                                         void *restrict pOut,
                                                         void *restrict pScratch);
 ...
 */
template <typename dataType>
extern AUDIOLIB_STATUS AUDIOLIB_delay_exec_cn(AUDIOLIB_kernelHandle handle,
                                              void *restrict pIn,
                                              void *restrict pDelay,
                                              void *restrict pOut,
                                              void *restrict pScratch);
...
typedef struct {
    ...
    *         @ref AUDIOLIB_delay_exec_cn and
    *         @ref AUDIOLIB_delay_exec_ci.                        */
   pFxnAUDIOLIB_delay_exec execute;
   ...
   uint32_t interleave;
   uint32_t delaySize;     /**< \brief Number of Samples in Delay buffer. */
   ...
   int32_t readIdx;  /**< \brief Read index for Circular Delay buffer. */
   int32_t writeIdx; /**< \brief Write index for Circular Delay buffer. */

   int32_t nVecsSamples;   /**< \brief Number of samples in a vector. */
   int32_t nVecsDelaySize; /**< \brief Number of delay samples in a vector. */
   int32_t nVecsDiff;      /**< \brief Number of difference samples in a vector. */

   /*! @brief bufPblock array to store SE/SA template */
   uint8_t bufPblock[AUDIOLIB_DELAY_IXX_IXX_OXX_PBLOCK_SIZE];

} AUDIOLIB_delay_PrivArgs;

#endif /* AUDIOLIB_DELAY_IXX_IXX_OXX_PRIV_H_ */
