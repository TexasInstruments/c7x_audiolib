...
void AUDIOLIB_dB20_perfEst(AUDIOLIB_kernelHandle handle, uint64_t *archCycles, uint64_t *estCycles)
{
   ...
   uint64_t dB20StartupCycles   = 0;
   uint64_t dB20TeardownCycles  = 0;
   uint64_t dB20OverheadCycles  = 0;
   uint32_t nVecs               = 0;
   uint64_t dB20OperationCycles = 0;

#if defined(__C7524__)
   {
      if (dataType == AUDIOLIB_FLOAT64) {
         if (numSamples == 1 && numChannels == 1) {
            dB20OperationCycles = 90;
         }
         else if (numSamples < 32) {
            dB20StartupCycles  = 18;
            dB20TeardownCycles = 3;

            nVecs               = pKerPrivArgs->nVecs;
            dB20OperationCycles = (111 + nVecs * 28) + (31 + nVecs * 7);
         }
         else {
            dB20StartupCycles  = 22;
            dB20TeardownCycles = 4;

            nVecs               = pKerPrivArgs->nVecs;
            dB20OperationCycles = (47 + nVecs * 10) + (63 + nVecs * 14) + (31 + nVecs * 7);
         }
      }
      else if (dataType == AUDIOLIB_FLOAT32) {
         if (numSamples == 1) {
            if (numChannels == 1) {
               dB20OperationCycles = 49;
            }
            else {
               nVecs               = numChannels * numSamples;
               dB20OperationCycles = (67 + nVecs * 12);
            }
         }
         else {
            dB20StartupCycles  = 10;
            dB20TeardownCycles = 1;

            nVecs               = pKerPrivArgs->nVecs;
            dB20OperationCycles = (47 + nVecs * 23);
         }
      }
   }
#elif defined(__C7504__)
   {
      ...
      if (dataType == AUDIOLIB_FLOAT32) {
         ...
         if (numSamples == 1) {
            ...
            dB20OperationCycles = (50 + nVecs * 20);
         }
         else {
            dB20OperationCycles = (62 + nVecs * 14);
         }
      }
   }
#endif

   dB20OverheadCycles = dB20StartupCycles + dB20TeardownCycles;
   *estCycles         = dB20OperationCycles + dB20OverheadCycles;
   *archCycles        = dB20OperationCycles;
}

template <typename dataType>
AUDIOLIB_STATUS AUDIOLIB_dB20_init_ci(AUDIOLIB_kernelHandle         handle,
...
template AUDIOLIB_STATUS AUDIOLIB_dB20_init_ci<float>(AUDIOLIB_kernelHandle         handle,
                                                      const AUDIOLIB_bufParams2D_t *bufParamsIn,
                                                      const AUDIOLIB_bufParams2D_t *bufParamsOut,
                                                      const AUDIOLIB_dB20_InitArgs *pKerInitArgs);

template AUDIOLIB_STATUS AUDIOLIB_dB20_init_ci<double>(AUDIOLIB_kernelHandle         handle,
                                                       const AUDIOLIB_bufParams2D_t *bufParamsIn,
                                                       const AUDIOLIB_bufParams2D_t *bufParamsOut,
                                                       const AUDIOLIB_dB20_InitArgs *pKerInitArgs);

inline double divdpMod_dB20dpi(double a, double b) { return cmn_DIVDP(a, b); }

template <typename dataType> static inline dataType AUDIOLIB_dB20_scalar_ci(dataType a);

template <> inline double AUDIOLIB_dB20_scalar_ci(double a)
{
   double       Half   = 0.5;
   ...
   X  = divdpMod_dB20dpi(zn, zd);
   W  = X * X;
   Bd = ((((W + b2) * W) + b1) * W) + b0;
   Cn = (((W * a2) + a1) * W) + a0;
   Rz = W * divdpMod_dB20dpi(Cn, Bd);
   Sa = X + (X * Rz);
   Cn = (double) N;
   Da = ((Cn * c2) + Sa) + (Cn * c1);
   Da = c10e * Da;
   ...
   if (Y < MINe) {
      Da = CONST_DOUBLE_MIN_DB20; // out[in0 <= 0.0] = np.uint64(0xFFF0000000000000)
   }
   if (Y > MAXe) {
      Da = CONST_MAX_DB20;
   }
...
template <> inline float AUDIOLIB_dB20_scalar_ci(float a)
{
...
AUDIOLIB_dB20_scaler_sp_1xN_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (uint32_t idx = 0; idx < pKerPrivArgs->nVecs; idx++) {
      pDst[idx] = AUDIOLIB_dB20_scalar_ci<dataType>(pSrc[idx]);
   }
   return status;
}

template AUDIOLIB_STATUS
AUDIOLIB_dB20_scaler_sp_1xN_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
AUDIOLIB_dB20_scaler_sp_MxN_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (uint32_t i = 0; i < pKerPrivArgs->nVecs; i++) {
      pDst[outOffset + idx] = AUDIOLIB_dB20_scalar_ci<dataType>(pSrc[inOffset + idx]);
      idx++;
...
template AUDIOLIB_STATUS
AUDIOLIB_dB20_scaler_sp_MxN_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);

#elif defined(__C7524__)
template <typename dataType>
AUDIOLIB_STATUS AUDIOLIB_dB20_vector_sp_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   AUDIOLIB_dB20_PrivArgs *pKerPrivArgs = (AUDIOLIB_dB20_PrivArgs *) handle;
   AUDIOLIB_STATUS status = AUDIOLIB_SUCCESS;
   uint8_t *pBlock = pKerPrivArgs->bufPblock;
   ...
   /***********************************************************************/
   /* Create and assign values for constants employed on dB20 computation */
   /***********************************************************************/
   ...
   // compute loop to perform vector dB20
   for (size_t i = 0; i < numBlocks; i++) {
      vec inVec = c7x::strm_eng<0, vec>::get_adv();

      /**********************************************************************/
      /* Create variables employed on dB20 computation                      */
      /**********************************************************************/
      vec pol, r1, r2, r3, r4;
      ...
      /**********************************************************************/
      /* Calculate Taylor series approximation for dB20                     */
      /**********************************************************************/
...
template AUDIOLIB_STATUS
AUDIOLIB_dB20_vector_sp_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
#endif
...
template AUDIOLIB_STATUS
AUDIOLIB_dB20_vector_dp_split1_exec_ci<double>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
template AUDIOLIB_STATUS
AUDIOLIB_dB20_vector_dp_split2_exec_ci<double>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);

template <typename dataType>
AUDIOLIB_STATUS
AUDIOLIB_dB20_scaler_1x1_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut) // 1x1
{
   AUDIOLIB_STATUS status = AUDIOLIB_SUCCESS;
   ...
   pDst[0] = AUDIOLIB_dB20_scalar_ci<dataType>(pSrc[0]);

   return status;
}

template AUDIOLIB_STATUS
AUDIOLIB_dB20_scaler_1x1_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);

template AUDIOLIB_STATUS
AUDIOLIB_dB20_scaler_1x1_exec_ci<double>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
...
AUDIOLIB_STATUS AUDIOLIB_dB20_scaler_Mx1_exec_ci(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (uint32_t idx = 0; idx < numChannel; idx++) {
      pDst[outOffset] = AUDIOLIB_dB20_scalar_ci<dataType>(pSrc[inOffset]);
      outOffset += outStride;
      inOffset += inStride;
   }
...
template AUDIOLIB_STATUS
AUDIOLIB_dB20_scaler_Mx1_exec_ci<float>(AUDIOLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut);
