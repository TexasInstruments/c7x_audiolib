...
#ifndef AUDIOLIB_GAIN_IXX_IXX_OXX_PRIV_H_
#define AUDIOLIB_GAIN_IXX_IXX_OXX_PRIV_H_

#include "../common/AUDIOLIB_utility.h"
#include "AUDIOLIB_gain.h"

/*!
 * @brief Macro to define the size of bufPblock array of
 *        @ref AUDIOLIB_gain_PrivArgs structure.
 *
 */

#define AUDIOLIB_GAIN_IXX_IXX_OXX_PBLOCK_SIZE (2 * SE_PARAM_SIZE)

/*!
 *  @brief This is a function pointer type that conforms to the
 *         declaration of @ref AUDIOLIB_gain_exec_ci
 *         and @ref AUDIOLIB_gain_exec_cn.
 */
typedef AUDIOLIB_STATUS (*pFxnAUDIOLIB_gain_exec)(AUDIOLIB_kernelHandle handle,
                                                  void *restrict pIn,
                                                  void *restrict pGain,
                                                  void *restrict pOut);
 ...
 *         to the declaration of @ref AUDIOLIB_gain_init.
 *
 * @details This function determines the configuration for the streaming engine
 *          and MMA hardware resources based on the function call parameters,
 *          and the configuration is saved in bufPBlock array. In the kernel
 *          call sequence, @ref AUDIOLIB_gain_exec_ci would be
 *          called later independently by the application. When
 *          @ref AUDIOLIB_gain_exec_ci runs, it merely retrieves
 *          the configuration from the bufPBlock and uses it to set up the
 *          hardware resources. This arrangement is so that
 *          @ref AUDIOLIB_gain_exec_ci does not lose cycles
 ...
 *         implementation of the kernel. The function declaration conforms
 *         to the declaration of @ref AUDIOLIB_gain_exec.
 *
 ...
 *  @param [in]  pGain       : Pointer to buffer holding the gain
...
typedef struct {
    ...
    *         @ref AUDIOLIB_gain_exec_cn and
    *         @ref AUDIOLIB_gain_exec_ci.                        */
   pFxnAUDIOLIB_gain_exec execute;
   ...
   int32_t  strideIn;  /**< \brief input Stride in Y dimension in bytes. */
   int32_t  strideOut; /**< \brief output Stride in Y dimension in bytes. */

   /*! @brief bufPblock array to store SE/SA template */
   uint8_t bufPblock[AUDIOLIB_GAIN_IXX_IXX_OXX_PBLOCK_SIZE];

} AUDIOLIB_gain_PrivArgs;
