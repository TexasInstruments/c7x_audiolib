                                  ...
                                  uint32_t              strideOut)
{
   ...
   uint64_t gainNChTrimStartupCycles   = 0;
   uint64_t gainNChTrimTeardownCycles  = 0;
   uint64_t gainNChTrimOperationCycles = 0;
   uint64_t gainNChTrimOverheadCycles  = 0;
   uint32_t nVecs                      = 0;

   if (pKerPrivArgs->isInterleave == 0) {
      gainNChTrimStartupCycles = 26;
      nVecs                    = pKerPrivArgs->nVecs;
      if (dataType == AUDIOLIB_FLOAT32) {
         gainNChTrimOperationCycles = 8 + nVecs * 1;
      }
      else {
         gainNChTrimOperationCycles = 8 + nVecs * 2;
      }
      gainNChTrimTeardownCycles = 2;
   }
   else {
      if (pKerPrivArgs->customImplementation) {
         if (dataType == AUDIOLIB_FLOAT32) {
            gainNChTrimStartupCycles   = 13;
            nVecs                      = pKerPrivArgs->nVecs;
            gainNChTrimOperationCycles = 12 + nVecs * 1;
            gainNChTrimTeardownCycles  = 3;
         }
         else {
            gainNChTrimStartupCycles   = 13;
            nVecs                      = pKerPrivArgs->nVecs;
            gainNChTrimOperationCycles = 12 + nVecs * 2;
            gainNChTrimTeardownCycles  = 3;
         }
      }
      else {
         if (dataType == AUDIOLIB_FLOAT32) {
            gainNChTrimStartupCycles   = 25;
            nVecs                      = pKerPrivArgs->nVecs;
            gainNChTrimOperationCycles = 8 + nVecs * 1;
            gainNChTrimTeardownCycles  = 3;
         }
         else {
            gainNChTrimStartupCycles   = 15;
            nVecs                      = pKerPrivArgs->nVecs;
            gainNChTrimOperationCycles = 8 + nVecs * 2;
            gainNChTrimTeardownCycles  = 3;
         }
   ...
   gainNChTrimOverheadCycles += gainNChTrimStartupCycles + gainNChTrimTeardownCycles;
   *estCycles  = gainNChTrimOperationCycles + gainNChTrimOverheadCycles;
   *archCycles = gainNChTrimOperationCycles;
}
                                             ...
                                             const AUDIOLIB_gainNChTrim_InitArgs *pKerInitArgs)
{
   ...
   AUDIOLIB_DEBUGPRINTFN(0, "Enter AUDIOLIB_gainNChTrim_init_ci \n");
...
template AUDIOLIB_STATUS AUDIOLIB_gainNChTrim_init_ci<float>(AUDIOLIB_kernelHandle                handle,
                                                             const AUDIOLIB_bufParams2D_t        *bufParamsIn,
                                                             const AUDIOLIB_bufParams1D_t        *bufParamsGain,
                                                             const AUDIOLIB_bufParams2D_t        *bufParamsOut,
                                                             const AUDIOLIB_gainNChTrim_InitArgs *pKerInitArgs);

template AUDIOLIB_STATUS AUDIOLIB_gainNChTrim_init_ci<double>(AUDIOLIB_kernelHandle                handle,
                                                              const AUDIOLIB_bufParams2D_t        *bufParamsIn,
                                                              const AUDIOLIB_bufParams1D_t        *bufParamsGain,
                                                              const AUDIOLIB_bufParams2D_t        *bufParamsOut,
                                                              const AUDIOLIB_gainNChTrim_InitArgs *pKerInitArgs);
...
template <typename dataType>
inline void AUDIOLIB_evalGain(__SE_TEMPLATE_v1 se0Params,
                              ...
                              uint32_t nVecs)
{
   ...
   vec masterGain = (vec) (*pMasterGainLocal);

   for (uint32_t counter = 0; counter < nVecs; counter++) {

      vec gainNChTrim = c7x::strm_eng<1, vec>::get_adv();
      vec inVec       = c7x::strm_eng<0, vec>::get_adv();

      vec outVec = gainNChTrim * inVec * masterGain;
                                             ...
                                             void *restrict pOut)
{
   ...
   AUDIOLIB_DEBUGPRINTFN(0, "Enter AUDIOLIB_gainNChTrim_exec_ci nVecs %d\n", pKerPrivArgs->nVecs);
   ...
   AUDIOLIB_evalGain<dataType>(se0Params, se1Params, sa0Params, pOut, pIn, pGain, pMasterGain, nVecs);

   return status;
}

template AUDIOLIB_STATUS AUDIOLIB_gainNChTrim_exec_ci<float>(AUDIOLIB_kernelHandle handle,
                                                             void *restrict pIn,
                                                             void *restrict pGain,
                                                             void *restrict pMasterGain,
                                                             void *restrict pOut);

template AUDIOLIB_STATUS AUDIOLIB_gainNChTrim_exec_ci<double>(AUDIOLIB_kernelHandle handle,
                                                              void *restrict pIn,
                                                              void *restrict pGain,
                                                              void *restrict pMasterGain,
                                                              void *restrict pOut);
